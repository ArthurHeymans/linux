...
pub const DTCM_INITIALIZED_PREFIX_SIZE: usize = 0x2078;
pub const DTCM_RUNTIME_SUFFIX_SIZE: usize = 0x9c44 - DTCM_INITIALIZED_PREFIX_SIZE;
...
/// Fourth band/calibration register-write list at 0x04000c60, applied by
/// vendor `phy_apply_reg_init_lists` via `reg_write_list_apply`
/// (0xFFFFFFFF address terminator; seven pairs verified in the COPY
/// snapshot, last pair 0ABA8540 = 0000017F).
#[repr(C, align(4))] struct PhyInitRegisterWriteList3 { writes: [RegisterWrite; 7], terminator_address: SharedU32, terminator_opaque: SharedU32 } /// Vendor boot banner at [0x040009de, 0x04000b28). NOT YET PORTED: the Rust
/// image has its own console path and never reads these bytes.
///
/// Vendor behavior (`dbg_print_banner`, PC 0x16012, runs once during boot):
/// it enables the console state byte at 0x0400973c+0x14, then polls the
/// UART status word at MMIO 0x9c500000 until bit 19 (TX ready) clears, and
/// finally sends this table byte-by-byte to the TX register 0x9c50008 --
/// indices 1..=0x149 over the table base (loop bound 0x14a at PC 0x1602a);
/// index 0 is skipped by that loop. The content is vendor-encoded, not
/// ASCII; decoding belongs to whatever host-side console consumes it.
#[repr(C, align(2))] struct VendorDebugEncodedBanner { encoded_banner: OpaqueBytes<0x14a> }
/// Exception-reason name pointers at [0x04000b28, 0x04000b3c). NOT YET
/// PORTED: src/exception.rs implements its own quarantine record instead of
/// the vendor WSM exception-indication path.
///
/// Vendor behavior (`exc_print_dump`, PC 0x161e8): for an exception payload
/// with reason < 5 it prints "Exception Reason: %s" using the pointer at
/// `names[reason]`; reason 4 additionally prints "Assert at File %s,
/// Error %u" from payload fields. The five pointers target vendor-.text
/// strings: UNDEF_INSTR, PREFETCH_ABORT, DATA_ABORT, UNKNOWN_ERROR, ASSERT.
#[repr(C, align(4))] struct ExceptionReasonNames { names: [SharedU32; 5] }
/// Hardware-timer and channel constants at [0x04000b3c, 0x04000b60). NOT
/// YET PORTED.
///
/// Vendor behavior: a boot-time timer routine (PC 0x5794) reads halfwords
/// at 0x04000b3e + idx * 4 -- the upper half of each 4-byte group in
/// `divisor_table` -- as divisors against a tick count via __udivsi3;
/// `phy_set_channel_full` (PC 0x167ec) performs `ldm r0, {r0, r1}` over
/// `channel_config_words` and may substitute an alternate second word from
/// a nearby constant depending on a band latch.
#[repr(C, align(4))] struct HwTimerDebugTables { divisor_table: OpaqueBytes<0x1c>, channel_config_words: [SharedU32; 2] }
/// Vendor debug-console strings at [0x040007a4, 0x04000804). NOT YET
/// PORTED: the Rust image has its own console.
///
/// Vendor behavior: `dbg_console_execute` (PC 0x15b10) prints
/// `console_help_text` verbatim -- "Commands are case sensitive. A comment
/// starts with # and ends at end-of-line.\n" followed by a NUL and padding.
/// `hex_digits` holds "0123456789ABCDEF"; no direct decoded load was found
/// for it (the console's hex formatting presumably indexes it relative to
/// the same base), so its consumer is presumed but unproven.
#[repr(C, align(4))] struct DebugConsoleStrings { console_help_text: OpaqueBytes<0x50>, hex_digits: OpaqueBytes<0x10> }
/// Vendor encoded help-text blob at [0x04000830, 0x040009de). NOT YET
/// PORTED; no AES relationship -- the long-standing "mode1 microcode" name
/// was a misnomer.
///
/// Vendor behavior (`dbg_print_help_text`, PC 0x15fe0 -- the console's "#"
/// help command): identical mechanism to `dbg_print_banner` -- enables the
/// console state byte at 0x0400973c+0x14, polls the UART status word at
/// MMIO 0x9c500000 for bit 19, then sends this table byte-by-byte to TX
/// register 0x9c50008 -- indices 1..=0x1ad over the base (loop bound 0x1ae
/// at PC 0x15ffa); index 0 is skipped. Content is vendor-encoded, not
/// ASCII.
#[repr(C, align(2))] struct VendorEncodedHelpText { encoded_help_text: OpaqueBytes<0x1ae> } /* Retained code interprets lower and upper as signed 16-bit values. */ #[repr(C, align(2))] struct InitializedPhyGainSourceRecord { selector: SharedU8, opaque_01: SharedU8, lower: SharedU16, upper: SharedU16 } #[repr(C, align(4))] struct InitializedIqCalibrationGainIndices { entries: [SharedU32; 12] } #[repr(C, align(4))] struct MeasurementWorkspace { dwell_bound: SharedU16, opaque_02: OpaqueBytes<0x03>, measurement_type: SharedU8, opaque_06: OpaqueBytes<0x02>, completion_status: SharedU32, opaque_0c: OpaqueBytes<0x05>, dispatch_state: SharedU8, dispatch_argument: SharedU16, opaque_14: OpaqueBytes<0x04>, start_timestamp_words: [SharedU32; 2], elapsed_timestamp_words: [SharedU32; 2], scan_request_prefix: OpaqueBytes<0x01>, scan_request_mode: SharedU8, opaque_2a: OpaqueBytes<0x3e> } #[repr(C, align(4))] struct TxAggregateExpirationDelta { value: SharedU32 } #[repr(C, align(4))] struct DebugCommandDescriptor { command_name: SharedU32, help_text: SharedU32, handler: SharedU32 } #[repr(C, align(4))] struct InitializedDebugCommandDescriptors { records: [DebugCommandDescriptor; 6] } #[repr(C, align(4))] struct PhyWatchdogCounter { count: SharedU32 } #[repr(C, align(4))] struct InitializedMultiVifBeaconTimerTail { opaque_00: OpaqueBytes<0x0f>, interface_2_radio_latch: SharedU8, timer: TimerEntry, opaque_24: OpaqueBytes<0x18>, measurement_dwell_timer: TimerEntry, dtim_capture_latch: SharedU8, opaque_51: OpaqueBytes<0x03>, measurement_control_word_0: SharedU32, measurement_control_word_1: SharedU32, measurement_control_word_2: SharedU32 } #[repr(C, align(4))] struct RetryPathCounter { count: SharedU32 } #[repr(C, align(4))] struct PerTidTelemetryBank { table_00: [SharedU32; 8], table_01: [SharedU32; 8], table_02: [SharedU32; 8], table_03: [SharedU32; 8], table_04: [SharedU32; 8], table_05: [SharedU32; 8], table_06: [SharedU32; 8], table_07: [SharedU32; 8], table_08: [SharedU32; 8], table_09: [SharedU32; 8] } #[repr(C, align(4))] struct TxConfirmAggregationState { state: SharedU32, pending_message_raw: SharedU32, append_cursor_raw: SharedU32 } #[repr(C, align(4))] struct ConfigurationApplyFlags { flags: SharedU32 } #[repr(C, align(4))] struct DebugPlatformLocalTail { platform_local_word_2c: SharedU32, platform_local_word_30: SharedU32 } #[repr(C, align(4))] struct PreHostPasRingObserved { opaque_00: OpaqueBytes<0x06>, radio_stop_word_02: SharedU16, opaque_08: OpaqueBytes<0x04> } #[repr(C, align(4))] struct TlvDispatchHandlerEntry { key: SharedU32, handler: SharedU32 } #[repr(C, align(4))] struct TlvDispatchHandlerTable { entries: [TlvDispatchHandlerEntry; 4], terminator_key: SharedU32, terminator_handler: SharedU32 } #[repr(C, align(2))] struct RfModeHalfwordTable { entries: [SharedU16; 36] } #[repr(C, align(4))] struct PhyCalSubstateRegisterWriteList { writes: [RegisterWrite; 8], terminator_address: SharedU32, terminator_opaque: SharedU32 } #[repr(C, align(4))] struct DbgExpandRegisterWriteList { writes: [RegisterWrite; 5], terminator_address: SharedU32, terminator_opaque: SharedU32 } #[repr(C, align(2))] struct RfScaleHalfwordTable { entries: [SharedU16; 64] } #[repr(C, align(2))] struct TxGainRssiHalfwordTable { entries: [SharedU16; 22] } #[repr(C, align(4))] struct PsWakeGuardWords { word_10e4: SharedU32, opaque_04: OpaqueBytes<0x04>, word_10ec: SharedU32, word_10f0: SharedU32, opaque_10: OpaqueBytes<0x04> } #[repr(C, align(4))] struct MacSlotTimingPatchEntry { pointer: SharedU32, patch_word: SharedU32 } #[repr(C, align(4))] struct MacSlotTimingPatchList { entries: [MacSlotTimingPatchEntry; 11] } #[repr(C, align(4))] struct PacDurationQuanta { quanta: [SharedU32; 8] } #[repr(C, align(4))] struct RatePairTable { pairs: [[SharedU8; 2]; 31], opaque_3e: OpaqueBytes<0x02> } #[repr(C, align(4))] struct SchedulerTail { opaque_2018: OpaqueBytes<0x10>, hardware_timer_guard: SharedU32, rf_calibration_bytes: [SharedU8; 32], opaque_204c: OpaqueBytes<0x04>, error_event_counts: [SharedU32; 10] } #[repr(C, align(4))] struct MacAggregateSlotTables { queues: [[SharedU32; 16]; 8] } #[repr(C, align(4))] struct MacPipeTail { setup_word_6dc: SharedU16, setup_word_6de: SharedU16, setup_word_6e0: SharedU16, opaque_6e2: OpaqueBytes<0x02>, opaque_6e4: OpaqueBytes<0x1c>, type_byte_700: SharedU8, opaque_701: OpaqueBytes<0x03>, cleared_word_704: SharedU32, counter_word_708: SharedU32, counter_word_70c: SharedU32, counter_word_710: SharedU32, counter_word_714: SharedU32, frame_count_718: SharedU32, scratch_word_71c: SharedU32 } #[repr(C, align(4))] struct MacPipeTails { records: [MacPipeTail; 4] }
/// Zero-initialized DTCM prefix with reconstructed families represented at
/// their qualified offsets. Bytes between families remain explicit opaque data.
#[repr(C, align(4))]
struct InitializedDtcmPrefix {
    mac_slot_timing_patch_list: MacSlotTimingPatchList, pac_duration_quanta: PacDurationQuanta, prefix_suffix: OpaqueBytes<0xc0>,
    ...
    ring_cursor_map_tables: RingCursorMapTables, queue_pipe_mappings: QueuePipeMappings,
    ...
    /// Vendor debug/exception tables (banner, exception-reason names,
    /// timer/channel constants). Not yet ported; see the struct docs.
    vendor_debug_encoded_banner: VendorDebugEncodedBanner, exception_reason_names: ExceptionReasonNames, hw_timer_debug_tables: HwTimerDebugTables,
    phy_gain_register_write_lists: [RegisterWriteList; 2],
    phy_init_register_write_list_0: PhyInitRegisterWriteList0, phy_init_register_write_list_1: PhyInitRegisterWriteList1, phy_init_register_write_list_2: PhyInitRegisterWriteList2, phy_init_register_write_list_3: PhyInitRegisterWriteList3, initialized_phy_gain_source_records: [InitializedPhyGainSourceRecord; 44], tlv_dispatch_handler_table: TlvDispatchHandlerTable, rf_mode_halfword_table: RfModeHalfwordTable, initialized_iq_calibration_gain_indices: InitializedIqCalibrationGainIndices, /* Retained rf_write_iq_corr_regs performs an unchecked u32 lookahead reading the first address word of the phy-cal substate list. */ phy_cal_substate_register_write_list: PhyCalSubstateRegisterWriteList, dbg_expand_register_write_list: DbgExpandRegisterWriteList, pre_rf_scale_tables: OpaqueBytes<0xc8>, rf_scale_table_a: RfScaleHalfwordTable, rf_scale_table_b: RfScaleHalfwordTable, rate_pointer_targets: OpaqueBytes<0x20>, tx_gain_rssi_halfword_table: TxGainRssiHalfwordTable,
    duration_quantum_pointers: [SharedU32; 4],
    ps_wake_guard_words: PsWakeGuardWords, measurement_workspace: MeasurementWorkspace, tx_aggregate_expiration_delta: TxAggregateExpirationDelta, debug_command_descriptors: InitializedDebugCommandDescriptors,
...
/// Vendor PAS static rate tables at [0x0400014c, 0x04000194), read-only
/// inputs to the vendor PAS rate machinery.
///
/// - `mcs_fallback_rates`: for MCS idx 14..=21, `pas_build_rate_tables`
///   reads the fallback rate index at 0x0400014c + (idx - 14)
///   (`ldrb r0, [r0, #0x12]` at PC 0x85e6 with r0 = DAT_00008724 - 0x5e +
///   idx - 0x20 = 0x0400012c + idx).
/// - `ofdm_airtime_durations`: frame-duration halfwords indexed by
///   `(rate - 6) & 0xf`; read by `airtime_compute` (DAT_00008724 - 0x4e =
///   0x0400015c) and `ofdm_calc_duration` (DAT_00000fdc = 0x0400015c). The
///   index mask proves the 16-entry extent.
/// - `rate_group_records`: byte-addressed records with a 4-byte stride;
///   `pas_program_rate_tables` reads bytes +0/+1/+2 of record `pair[0]`, and
///   `mac_program_ifs_timing` reads bytes +4/+8 (records 1 and 2's first
///   byte, DAT_0000fffc + iVar1*4).
#[repr(C, align(4))]
struct PasRateStaticTables {
    mcs_fallback_rates: [SharedU8; 8],
    /// No accessor found; initialized bytes are the identity sequence
    /// 0x0e..0x15. Left opaque.
    pas_fallback_suffix: OpaqueBytes<0x8>,
    ofdm_airtime_durations: [SharedU16; 16],
    rate_group_records: [SharedU32; 6],
}

/// Vendor completion-prefix region at [0x04000228, 0x04000260).
#[repr(C, align(4))]
struct CompletionCallbackPrefix {
    /// No accessor found for [0x04000228, 0x04000258).
    completion_prefix_suffix: OpaqueBytes<0x30>,
    /// `p2p_build_action_frame` reads `*(u8 *)(0x04000258 + (action >> 1))`
    /// as a frame-write offset (PC 0x3656). Only indices 0..=3 hold
    /// meaningful values (04 08 08 12); the remaining bytes are initialized
    /// zeros.
    p2p_action_offsets: [SharedU8; 8],
}

/// Vendor ring-cursor-map region at [0x04000288, 0x040002d8).
#[repr(C, align(4))]
struct RingCursorMapTables {
    /// `rx_beacon_update_erp_ht_flags` copies the last seven words to the
    /// stack (`fw_memcpy` from offset +4, length 0x1c, PC 0x4472) and tests
    /// each word individually (loop bound 7); the first word is never read.
    beacon_mask_words: [SharedU32; 8],
    /// `mib_defaults_copy` copies these 48 bytes wholesale to SRAM
    /// 0x04003a78 during boot (PC 0x84a).
    mib_defaults_template: OpaqueBytes<0x30>,
}
...
#[repr(C, align(4))]
struct MacPipeRecord {
    producer_slot: SharedU8,
    last_slot: SharedU8,
    current_slot: SharedU8,
    state: SharedU8,
    control: SharedU8,
    watchdog: SharedU8,
    abort_status: SharedU16,
    hardware_ring: SharedU32,
    slots: [MacPipeSlot; 4],
}
#[repr(C, align(4))]
...
#[repr(C, align(4))]
struct DtcmLayout {
    initialized_prefix: InitializedDtcmPrefix, // 0x0000
...
macro_rules! link_dtcm_data_region {
    ($symbol:ident, $section:literal, $ty:ty) => {
        #[unsafe(no_mangle)]
        #[used]
        #[unsafe(link_section = $section)]
        #[cfg(target_arch = "arm")]
        static $symbol: SharedDtcmRegion<$ty> =
            SharedDtcmRegion(UnsafeCell::new(MaybeUninit::uninit()));
    };
}

link_dtcm_data_region!(DTCM_DATA_MAC_SLOT_TIMING_PATCH_LIST, ".dtcm.bss.initialized.000_mac_slot_timing_patch_list", MacSlotTimingPatchList);
link_dtcm_data_region!(DTCM_DATA_PAC_DURATION_QUANTA, ".dtcm.bss.initialized.001_pac_duration_quanta", PacDurationQuanta);
link_dtcm_data_region!(DTCM_DATA_PREFIX_SUFFIX, ".dtcm.bss.initialized.002_prefix_suffix", OpaqueBytes<0xc0>);
link_dtcm_data_region!(DTCM_DATA_TX_DURATION_TIMING, ".dtcm.bss.initialized.003_tx_duration_timing", [SharedU16; 10]);
link_dtcm_data_region!(DTCM_DATA_PAS_RATE_STATIC_TABLES, ".dtcm.bss.initialized.004_pas_rate_static_tables", PasRateStaticTables);
link_dtcm_data_region!(DTCM_DATA_RATE_ENCODING, ".dtcm.bss.initialized.005_rate_encoding", [SharedU8; 22]);
link_dtcm_data_region!(DTCM_DATA_RATE_ATTRIBUTES, ".dtcm.bss.initialized.006_rate_attributes", [SharedU8; 22]);
link_dtcm_data_region!(DTCM_DATA_RATE_PAIR_TABLE, ".dtcm.bss.initialized.007_rate_pair_table", RatePairTable);
link_dtcm_data_region!(DTCM_DATA_INITIALIZED_RATE_POLICIES, ".dtcm.bss.initialized.008_initialized_rate_policies", InitializedRatePolicies);
link_dtcm_data_region!(DTCM_DATA_COMPLETION_CALLBACK_PREFIX, ".dtcm.bss.initialized.009_completion_callback_prefix", CompletionCallbackPrefix);
link_dtcm_data_region!(DTCM_DATA_VISIBLE_COMPLETION_WORDS, ".dtcm.bss.initialized.010_visible_completion_words", [SharedU32; 10]);
link_dtcm_data_region!(DTCM_DATA_RING_CURSOR_MAP_TABLES, ".dtcm.bss.initialized.011_ring_cursor_map_tables", RingCursorMapTables);
link_dtcm_data_region!(DTCM_DATA_QUEUE_PIPE_MAPPINGS, ".dtcm.bss.initialized.012_queue_pipe_mappings", QueuePipeMappings);
link_dtcm_data_region!(DTCM_DATA_PRE_TKIP_SBOX_TABLES, ".dtcm.bss.initialized.013_pre_tkip_sbox_tables", OpaqueBytes<0x2c>);
link_dtcm_data_region!(DTCM_DATA_TKIP_SBOX_TABLES, ".dtcm.bss.initialized.014_tkip_sbox_tables", TkipSboxTables);
link_dtcm_data_region!(DTCM_DATA_COMMAND_DISPATCH, ".dtcm.bss.initialized.015_command_dispatch", [SharedU32; 37]);
link_dtcm_data_region!(DTCM_DATA_DEBUG_CONSOLE_STRINGS, ".dtcm.bss.initialized.016_debug_console_strings", DebugConsoleStrings);
link_dtcm_data_region!(DTCM_DATA_AES_TRANSFER_CLASSES, ".dtcm.bss.initialized.017_aes_transfer_classes", AesTransferClassTable);
link_dtcm_data_region!(DTCM_DATA_VENDOR_ENCODED_HELP_TEXT, ".dtcm.bss.initialized.018_vendor_encoded_help_text", VendorEncodedHelpText);
link_dtcm_data_region!(DTCM_DATA_VENDOR_DEBUG_ENCODED_BANNER, ".dtcm.bss.initialized.019_vendor_debug_encoded_banner", VendorDebugEncodedBanner);
link_dtcm_data_region!(DTCM_DATA_EXCEPTION_REASON_NAMES, ".dtcm.bss.initialized.020_exception_reason_names", ExceptionReasonNames);
link_dtcm_data_region!(DTCM_DATA_HW_TIMER_DEBUG_TABLES, ".dtcm.bss.initialized.021_hw_timer_debug_tables", HwTimerDebugTables);
link_dtcm_data_region!(DTCM_DATA_PHY_GAIN_REGISTER_WRITE_LISTS, ".dtcm.bss.initialized.022_phy_gain_register_write_lists", [RegisterWriteList; 2]);
link_dtcm_data_region!(DTCM_DATA_PHY_INIT_REGISTER_WRITE_LIST_0, ".dtcm.bss.initialized.023_phy_init_register_write_list_0", PhyInitRegisterWriteList0);
link_dtcm_data_region!(DTCM_DATA_PHY_INIT_REGISTER_WRITE_LIST_1, ".dtcm.bss.initialized.024_phy_init_register_write_list_1", PhyInitRegisterWriteList1);
link_dtcm_data_region!(DTCM_DATA_PHY_INIT_REGISTER_WRITE_LIST_2, ".dtcm.bss.initialized.025_phy_init_register_write_list_2", PhyInitRegisterWriteList2);
link_dtcm_data_region!(DTCM_DATA_PHY_INIT_REGISTER_WRITE_LIST_3, ".dtcm.bss.initialized.026_phy_init_register_write_list_3", PhyInitRegisterWriteList3);
link_dtcm_data_region!(DTCM_DATA_INITIALIZED_PHY_GAIN_SOURCE_RECORDS, ".dtcm.bss.initialized.027_initialized_phy_gain_source_records", [InitializedPhyGainSourceRecord; 44]);
link_dtcm_data_region!(DTCM_DATA_TLV_DISPATCH_HANDLER_TABLE, ".dtcm.bss.initialized.028_tlv_dispatch_handler_table", TlvDispatchHandlerTable);
link_dtcm_data_region!(DTCM_DATA_RF_MODE_HALFWORD_TABLE, ".dtcm.bss.initialized.029_rf_mode_halfword_table", RfModeHalfwordTable);
link_dtcm_data_region!(DTCM_DATA_INITIALIZED_IQ_CALIBRATION_GAIN_INDICES, ".dtcm.bss.initialized.030_initialized_iq_calibration_gain_indices", InitializedIqCalibrationGainIndices);
link_dtcm_data_region!(DTCM_DATA_PHY_CAL_SUBSTATE_REGISTER_WRITE_LIST, ".dtcm.bss.initialized.031_phy_cal_substate_register_write_list", PhyCalSubstateRegisterWriteList);
link_dtcm_data_region!(DTCM_DATA_DBG_EXPAND_REGISTER_WRITE_LIST, ".dtcm.bss.initialized.032_dbg_expand_register_write_list", DbgExpandRegisterWriteList);
link_dtcm_data_region!(DTCM_DATA_PRE_RF_SCALE_TABLES, ".dtcm.bss.initialized.033_pre_rf_scale_tables", OpaqueBytes<0xc8>);
link_dtcm_data_region!(DTCM_DATA_RF_SCALE_TABLE_A, ".dtcm.bss.initialized.034_rf_scale_table_a", RfScaleHalfwordTable);
link_dtcm_data_region!(DTCM_DATA_RF_SCALE_TABLE_B, ".dtcm.bss.initialized.035_rf_scale_table_b", RfScaleHalfwordTable);
link_dtcm_data_region!(DTCM_DATA_RATE_POINTER_TARGETS, ".dtcm.bss.initialized.036_rate_pointer_targets", OpaqueBytes<0x20>);
link_dtcm_data_region!(DTCM_DATA_TX_GAIN_RSSI_HALFWORD_TABLE, ".dtcm.bss.initialized.037_tx_gain_rssi_halfword_table", TxGainRssiHalfwordTable);
link_dtcm_data_region!(DTCM_DATA_DURATION_QUANTUM_POINTERS, ".dtcm.bss.initialized.038_duration_quantum_pointers", [SharedU32; 4]);
link_dtcm_data_region!(DTCM_DATA_PS_WAKE_GUARD_WORDS, ".dtcm.bss.initialized.039_ps_wake_guard_words", PsWakeGuardWords);
link_dtcm_data_region!(DTCM_DATA_MEASUREMENT_WORKSPACE, ".dtcm.bss.initialized.040_measurement_workspace", MeasurementWorkspace);
link_dtcm_data_region!(DTCM_DATA_TX_AGGREGATE_EXPIRATION_DELTA, ".dtcm.bss.initialized.041_tx_aggregate_expiration_delta", TxAggregateExpirationDelta);
link_dtcm_data_region!(DTCM_DATA_DEBUG_COMMAND_DESCRIPTORS, ".dtcm.bss.initialized.042_debug_command_descriptors", InitializedDebugCommandDescriptors);
link_dtcm_data_region!(DTCM_DATA_HIF_CONTROL, ".dtcm.bss.initialized.043_hif_control", InitializedHifControl);
link_dtcm_data_region!(DTCM_DATA_IRQ_CALLBACKS, ".dtcm.bss.initialized.044_irq_callbacks", [SharedU32; 32]);
link_dtcm_data_region!(DTCM_DATA_PHY_WATCHDOG_COUNTER, ".dtcm.bss.initialized.045_phy_watchdog_counter", PhyWatchdogCounter);
link_dtcm_data_region!(DTCM_DATA_MULTI_VIF_BEACON_TIMER_TAIL, ".dtcm.bss.initialized.046_multi_vif_beacon_timer_tail", InitializedMultiVifBeaconTimerTail);
link_dtcm_data_region!(DTCM_DATA_AMPDU_COUNTERS, ".dtcm.bss.initialized.047_ampdu_counters", AmpduTelemetryCounters);
link_dtcm_data_region!(DTCM_DATA_RETRY_PATH_COUNTER, ".dtcm.bss.initialized.048_retry_path_counter", RetryPathCounter);
link_dtcm_data_region!(DTCM_DATA_PER_TID_TELEMETRY_BANK, ".dtcm.bss.initialized.049_per_tid_telemetry_bank", PerTidTelemetryBank);
link_dtcm_data_region!(DTCM_DATA_AMPDU_COMPLETION_CONTROL, ".dtcm.bss.initialized.050_ampdu_completion_control", AmpduCompletionControl);
link_dtcm_data_region!(DTCM_DATA_TX_CONFIRM_AGGREGATION_STATE, ".dtcm.bss.initialized.051_tx_confirm_aggregation_state", TxConfirmAggregationState);
link_dtcm_data_region!(DTCM_DATA_CONFIGURATION_APPLY_FLAGS, ".dtcm.bss.initialized.052_configuration_apply_flags", ConfigurationApplyFlags);
link_dtcm_data_region!(DTCM_DATA_CONTROL_WORDS, ".dtcm.bss.initialized.053_control_words", InitializedControlWords);
link_dtcm_data_region!(DTCM_DATA_PRE_PHY_CHANNEL_THRESHOLD_DESCRIPTORS, ".dtcm.bss.initialized.054_pre_phy_channel_threshold_descriptors", OpaqueBytes<0x14>);
link_dtcm_data_region!(DTCM_DATA_DEBUG_PLATFORM_LOCAL_TAIL, ".dtcm.bss.initialized.055_debug_platform_local_tail", DebugPlatformLocalTail);
link_dtcm_data_region!(DTCM_DATA_PHY_CHANNEL_THRESHOLD_DESCRIPTORS, ".dtcm.bss.initialized.056_phy_channel_threshold_descriptors", [PhyChannelThresholdDescriptor; 2]);
link_dtcm_data_region!(DTCM_DATA_PHY_GAIN_PROGRAMMING_RECORDS, ".dtcm.bss.initialized.057_phy_gain_programming_records", [PhyGainProgrammingRecord; 16]);
link_dtcm_data_region!(DTCM_DATA_PRE_HOST_PAS_RING, ".dtcm.bss.initialized.058_pre_host_pas_ring", PreHostPasRingObserved);
link_dtcm_data_region!(DTCM_DATA_HOST_PAS_RING, ".dtcm.bss.initialized.059_host_pas_ring", HostPasRing);
link_dtcm_data_region!(DTCM_DATA_INITIALIZED_LOW_MAC_PREFIX, ".dtcm.bss.initialized.060_initialized_low_mac_prefix", LowMacGlobalPrefix);
link_dtcm_data_region!(DTCM_DATA_MAC_PIPE_RECORDS, ".dtcm.bss.initialized.061_mac_pipe_records", [MacPipeRecord; 4]);
link_dtcm_data_region!(DTCM_DATA_MAC_TX_QUEUE_STATE, ".dtcm.bss.initialized.062_mac_tx_queue_state", MacTxQueueState);
link_dtcm_data_region!(DTCM_DATA_PRE_MAC_BEACON_STATE, ".dtcm.bss.initialized.063_pre_mac_beacon_state", OpaqueBytes<0x1a8>);
link_dtcm_data_region!(DTCM_DATA_MAC_BEACON_STATE, ".dtcm.bss.initialized.064_mac_beacon_state", MacBeaconState);
link_dtcm_data_region!(DTCM_DATA_MAC_WAKE_RUNTIME_STATE, ".dtcm.bss.initialized.065_mac_wake_runtime_state", MacWakeRuntimeState);
link_dtcm_data_region!(DTCM_DATA_PRE_MAC_PHY_COMMAND_STATE, ".dtcm.bss.initialized.066_pre_mac_phy_command_state", OpaqueBytes<0x08>);
link_dtcm_data_region!(DTCM_DATA_MAC_AGGREGATE_SLOT_TABLES, ".dtcm.bss.initialized.067_mac_aggregate_slot_tables", MacAggregateSlotTables);
link_dtcm_data_region!(DTCM_DATA_MAC_PHY_COMMAND_STATE, ".dtcm.bss.initialized.068_mac_phy_command_state", MacPhyCommandState);
link_dtcm_data_region!(DTCM_DATA_MAC_PIPE_TAILS, ".dtcm.bss.initialized.069_mac_pipe_tails", MacPipeTails);
link_dtcm_data_region!(DTCM_DATA_MAC_RETRY_HARDWARE_STATE, ".dtcm.bss.initialized.070_mac_retry_hardware_state", MacRetryHardwareState);
link_dtcm_data_region!(DTCM_DATA_PRE_MAC_RUNTIME_ACCOUNTING, ".dtcm.bss.initialized.071_pre_mac_runtime_accounting", OpaqueBytes<0x108>);
link_dtcm_data_region!(DTCM_DATA_MAC_RUNTIME_ACCOUNTING, ".dtcm.bss.initialized.072_mac_runtime_accounting", MacRuntimeAccountingState);
link_dtcm_data_region!(DTCM_DATA_SCHEDULER_EXCLUSION_STATE, ".dtcm.bss.initialized.073_scheduler_exclusion_state", SchedulerExclusionState);
link_dtcm_data_region!(DTCM_DATA_SCHEDULER_EVENT_ISLAND, ".dtcm.bss.initialized.074_scheduler_event_island", SchedulerEventIsland);
link_dtcm_data_region!(DTCM_DATA_INITIALIZED_TAIL, ".dtcm.bss.initialized.075_initialized_tail", SchedulerTail);
...
#[unsafe(no_mangle)]
#[used]
#[unsafe(link_section = ".dtcm.bss.host_tx_contexts")]
#[cfg(target_arch = "arm")]
static DTCM_HOST_TX_CONTEXTS: SharedDtcmRegion<HostTxContexts> =
    SharedDtcmRegion(UnsafeCell::new(MaybeUninit::uninit()));
...
#[unsafe(no_mangle)]
#[used]
#[unsafe(link_section = ".dtcm.bss.host_context_accounting")]
#[cfg(target_arch = "arm")]
static DTCM_HOST_CONTEXT_ACCOUNTING: SharedDtcmRegion<HostContextAccounting> =
    SharedDtcmRegion(UnsafeCell::new(MaybeUninit::uninit()));
...
#[unsafe(no_mangle)]
#[used]
#[unsafe(link_section = ".dtcm.bss.ba_sessions")]
#[cfg(target_arch = "arm")]
static DTCM_BA_SESSIONS: SharedDtcmRegion<BaSessions> =
    SharedDtcmRegion(UnsafeCell::new(MaybeUninit::uninit()));

#[unsafe(no_mangle)]
#[used]
#[unsafe(link_section = ".dtcm.bss.ba_link_event_state")]
#[cfg(target_arch = "arm")]
static DTCM_BA_LINK_EVENT_STATE: SharedDtcmRegion<BaLinkEventState> =
    SharedDtcmRegion(UnsafeCell::new(MaybeUninit::uninit()));

#[unsafe(no_mangle)]
#[used]
#[unsafe(link_section = ".dtcm.bss.tala")]
#[cfg(target_arch = "arm")]
static DTCM_TALA: SharedDtcmRegion<TalaAccounting> =
    SharedDtcmRegion(UnsafeCell::new(MaybeUninit::uninit()));

#[unsafe(no_mangle)]
#[used]
#[unsafe(link_section = ".dtcm.bss.context_completion_prefix")]
#[cfg(target_arch = "arm")]
static DTCM_CONTEXT_COMPLETION_PREFIX: SharedDtcmRegion<ContextCompletionPrefix> =
    SharedDtcmRegion(UnsafeCell::new(MaybeUninit::uninit()));
...
pub unsafe fn zero_initialized_data() {
    ...
    let (start, end) = (
        ...
        addr_of_mut!(__dtcm_data_end),
    );
    #[cfg(not(target_arch = "arm"))]
    let (start, end) = unsafe {
        let start = layout_ptr().cast::<u8>();
        (start, start.add(DTCM_INITIALIZED_PREFIX_SIZE))
    };
...
pub unsafe fn zero_runtime_bss() {
    ...
    #[cfg(not(target_arch = "arm"))]
    let (start, end) = unsafe {
        let start = layout_ptr().cast::<u8>().add(DTCM_INITIALIZED_PREFIX_SIZE);
        (start, start.add(DTCM_RUNTIME_SUFFIX_SIZE))
    };
...
/// Identity of one of the four fixed MAC TX pipe records.
#[repr(transparent)]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct MacPipeRecordAddress(DtcmAddress);

/// Identity of one of the four unchecked slots embedded in a MAC TX pipe.
#[repr(transparent)]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl MacPipeRecordAddress {
    ...
    pub(crate) const fn from_index_unchecked(pipe: usize) -> Self {
        Self(DtcmAddress::from_offset_unchecked(
            MAC_PIPE_RECORDS.offset() + pipe * core::mem::size_of::<MacPipeRecord>(),
        ))
    }
    ...
    pub(crate) const fn producer_slot(self) -> DtcmAddress { self.field(core::mem::offset_of!(MacPipeRecord, producer_slot)) }
    pub(crate) const fn last_slot(self) -> DtcmAddress { self.field(core::mem::offset_of!(MacPipeRecord, last_slot)) }
    pub(crate) const fn current_slot(self) -> DtcmAddress { self.field(core::mem::offset_of!(MacPipeRecord, current_slot)) }
    pub(crate) const fn state(self) -> DtcmAddress { self.field(core::mem::offset_of!(MacPipeRecord, state)) }
    pub(crate) const fn control(self) -> DtcmAddress { self.field(core::mem::offset_of!(MacPipeRecord, control)) }
    pub(crate) const fn watchdog(self) -> DtcmAddress { self.field(core::mem::offset_of!(MacPipeRecord, watchdog)) }
    pub(crate) const fn abort_status(self) -> DtcmAddress { self.field(core::mem::offset_of!(MacPipeRecord, abort_status)) }
    pub(crate) const fn hardware_ring(self) -> DtcmAddress { self.field(core::mem::offset_of!(MacPipeRecord, hardware_ring)) }
    pub(crate) const fn slot_unchecked(self, slot: usize) -> MacPipeSlotAddress {
        MacPipeSlotAddress(DtcmAddress::from_offset_unchecked(
            self.0.offset()
                + core::mem::offset_of!(MacPipeRecord, slots)
                + slot * core::mem::size_of::<MacPipeSlot>(),
        ))
    }
}
...
pub(crate) const fn mac_slot_timing_patch_pointer(index: usize) -> Option<DtcmAddress> { if index < 11 { Some(DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, mac_slot_timing_patch_list) + core::mem::offset_of!(MacSlotTimingPatchList, entries) + index * core::mem::size_of::<MacSlotTimingPatchEntry>() + core::mem::offset_of!(MacSlotTimingPatchEntry, pointer))) } else { None } }
pub(crate) const fn mac_slot_timing_patch_word(index: usize) -> Option<DtcmAddress> { if index < 11 { Some(DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, mac_slot_timing_patch_list) + core::mem::offset_of!(MacSlotTimingPatchList, entries) + index * core::mem::size_of::<MacSlotTimingPatchEntry>() + core::mem::offset_of!(MacSlotTimingPatchEntry, patch_word))) } else { None } }
pub const TKIP_SBOX_TABLES: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, tkip_sbox_tables)); pub(crate) const AES_TRANSFER_CLASSES: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, aes_transfer_classes)); pub(crate) const PHY_GAIN_REGISTER_WRITE_LISTS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, phy_gain_register_write_lists)); pub(crate) const PHY_INIT_REGISTER_WRITE_LISTS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, phy_init_register_write_list_0)); pub(crate) const INITIALIZED_PHY_GAIN_SOURCE_RECORDS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, initialized_phy_gain_source_records)); pub(crate) const INITIALIZED_IQ_CALIBRATION_GAIN_INDICES: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, initialized_iq_calibration_gain_indices)); pub(crate) const RF_MODE_HALFWORD_TABLE: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, rf_mode_halfword_table) + core::mem::offset_of!(RfModeHalfwordTable, entries));
pub(crate) const RF_SCALE_TABLE_A_MODE0_TARGET: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_a) + 60 * core::mem::size_of::<SharedU16>());
...
pub(crate) const PHY_GAIN_PROGRAMMING_RECORDS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, phy_gain_programming_records));
...
pub(crate) const MAC_PHY_COMMAND_STATE: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, mac_phy_command_state));
...
pub(crate) const MAC_PIPE_TAILS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, mac_pipe_tails));
...
pub(crate) const MAC_WAKE_RUNTIME_STATE: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, mac_wake_runtime_state));
...
/// Vendor secondary-rate overlay rooted at low-MAC-global `+0x47a`; callers
...
pub(crate) const LOW_MAC_GLOBAL: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, initialized_low_mac_prefix));
pub(crate) const LOW_MAC_FIFO_CONTROL: DtcmAddress = DtcmAddress::from_offset(LOW_MAC_GLOBAL.offset() + core::mem::offset_of!(LowMacGlobalPrefix, fifo_control));
...
pub(crate) const LOW_MAC_CONTROL_04: DtcmAddress = DtcmAddress::from_offset(LOW_MAC_GLOBAL.offset() + core::mem::offset_of!(LowMacGlobalPrefix, control_04));
...
pub(crate) const LOW_MAC_SLOT_TIME_X2: DtcmAddress = DtcmAddress::from_offset(LOW_MAC_GLOBAL.offset() + core::mem::offset_of!(LowMacGlobalPrefix, slot_time_x2));
pub(crate) const LOW_MAC_SLOT_TIME_X3: DtcmAddress = DtcmAddress::from_offset(LOW_MAC_GLOBAL.offset() + core::mem::offset_of!(LowMacGlobalPrefix, slot_time_x3));
pub(crate) const LOW_MAC_SLOT_TIME_CONSTANT: DtcmAddress = DtcmAddress::from_offset(LOW_MAC_GLOBAL.offset() + core::mem::offset_of!(LowMacGlobalPrefix, slot_time_constant));
...
pub(crate) const MAC_PIPE_RECORDS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, mac_pipe_records));
pub(crate) const fn mac_pipe_record(pipe: usize) -> Option<DtcmAddress> { match MacPipeRecordAddress::from_index(pipe) { Some(record) => Some(record.0), None => None } }
pub(crate) const fn mac_pipe_record_unchecked(pipe: usize) -> DtcmAddress { MacPipeRecordAddress::from_index_unchecked(pipe).0 }
const fn mac_pipe_record_field_unchecked(pipe: usize, offset: usize) -> DtcmAddress { MacPipeRecordAddress::from_index_unchecked(pipe).field(offset) }
pub(crate) const fn mac_pipe_current_slot_unchecked(pipe: usize) -> DtcmAddress { mac_pipe_record_field_unchecked(pipe, core::mem::offset_of!(MacPipeRecord, producer_slot)) }
pub(crate) const fn mac_pipe_cursor_mirror_01_unchecked(pipe: usize) -> DtcmAddress { mac_pipe_record_field_unchecked(pipe, core::mem::offset_of!(MacPipeRecord, last_slot)) }
pub(crate) const fn mac_pipe_cursor_mirror_02_unchecked(pipe: usize) -> DtcmAddress { mac_pipe_record_field_unchecked(pipe, core::mem::offset_of!(MacPipeRecord, current_slot)) }
...
pub(crate) const fn mac_pipe_control_byte_04_unchecked(pipe: usize) -> DtcmAddress { mac_pipe_record_field_unchecked(pipe, core::mem::offset_of!(MacPipeRecord, control)) }
pub(crate) const fn mac_pipe_control_byte_05_unchecked(pipe: usize) -> DtcmAddress { mac_pipe_record_field_unchecked(pipe, core::mem::offset_of!(MacPipeRecord, watchdog)) }
pub(crate) const fn mac_pipe_control_halfword_06_unchecked(pipe: usize) -> DtcmAddress { mac_pipe_record_field_unchecked(pipe, core::mem::offset_of!(MacPipeRecord, abort_status)) }
...
const fn mac_pipe_slot_field_unchecked(pipe: usize, slot: usize, offset: usize) -> DtcmAddress { MacPipeRecordAddress::from_index_unchecked(pipe).slot_unchecked(slot).field(offset) }
...
pub(crate) const RADIO_STOP_WORD_02: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, pre_host_pas_ring) + core::mem::offset_of!(PreHostPasRingObserved, radio_stop_word_02)); pub(crate) const HOST_PAS_RING: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, host_pas_ring));
...
pub(crate) const DURATION_QUANTUM_POINTERS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, duration_quantum_pointers));
...
pub(crate) const QUEUE_PIPE_MAPPINGS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, queue_pipe_mappings));
...
pub(crate) const INITIALIZED_CONTROL_WORDS: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, control_words));
...
pub const DTCM_RUNTIME_END: DtcmAddress = DtcmAddress::from_offset(0x9c44);
...
const fn scheduler_exclusion_field(offset: usize) -> DtcmAddress {
    DtcmAddress::from_offset_unchecked(
        core::mem::offset_of!(DtcmLayout, initialized_prefix)
            + core::mem::offset_of!(InitializedDtcmPrefix, scheduler_exclusion_state)
            + offset,
    )
}
const fn scheduler_event_field(offset: usize) -> DtcmAddress {
    DtcmAddress::from_offset_unchecked(
        core::mem::offset_of!(DtcmLayout, initialized_prefix)
            + core::mem::offset_of!(InitializedDtcmPrefix, scheduler_event_island)
            + offset,
    )
}
...
pub(crate) const SCHEDULER_HARDWARE_TIMER_GUARD: DtcmAddress = DtcmAddress::from_offset(core::mem::offset_of!(InitializedDtcmPrefix, initialized_tail) + core::mem::offset_of!(SchedulerTail, hardware_timer_guard));
...
const _: () = {
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, mac_slot_timing_patch_list) == 0x0000); assert_type_layout!(MacSlotTimingPatchEntry, 0x08, 4); assert!(core::mem::offset_of!(MacSlotTimingPatchEntry, pointer) == 0); assert!(core::mem::offset_of!(MacSlotTimingPatchEntry, patch_word) == 0x04); assert_type_layout!(MacSlotTimingPatchList, 0x58, 4); assert!(core::mem::offset_of!(MacSlotTimingPatchList, entries) == 0); assert!(core::mem::size_of::<[MacSlotTimingPatchEntry; 11]>() == 0x58); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, mac_slot_timing_patch_list) + core::mem::size_of::<MacSlotTimingPatchList>() == 0x0400_0058); assert!(core::mem::offset_of!(InitializedDtcmPrefix, pac_duration_quanta) == 0x0058); assert_type_layout!(PacDurationQuanta, 0x20, 4); assert!(core::mem::offset_of!(PacDurationQuanta, quanta) == 0); assert!(core::mem::size_of::<[SharedU32; 8]>() == 0x20); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, pac_duration_quanta) + core::mem::size_of::<PacDurationQuanta>() == 0x0400_0078); assert!(core::mem::offset_of!(InitializedDtcmPrefix, prefix_suffix) == 0x0078); assert!(core::mem::size_of::<OpaqueBytes<0xc0>>() == 0xc0); assert!(core::mem::offset_of!(InitializedDtcmPrefix, tx_duration_timing) == 0x0138); assert!(core::mem::offset_of!(InitializedDtcmPrefix, pas_rate_static_tables) == 0x014c); assert_type_layout!(PasRateStaticTables, 0x48, 4); assert!(core::mem::offset_of!(PasRateStaticTables, mcs_fallback_rates) == 0x00); assert!(core::mem::offset_of!(PasRateStaticTables, pas_fallback_suffix) == 0x08); assert!(core::mem::size_of::<OpaqueBytes<0x8>>() == 0x8); assert!(core::mem::offset_of!(PasRateStaticTables, ofdm_airtime_durations) == 0x10); assert!(core::mem::size_of::<[SharedU16; 16]>() == 0x20); assert!(core::mem::offset_of!(PasRateStaticTables, rate_group_records) == 0x30); assert!(core::mem::size_of::<[SharedU32; 6]>() == 0x18); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, pas_rate_static_tables) + core::mem::size_of::<PasRateStaticTables>() == 0x0400_0194);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, rate_encoding) == 0x0194);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, rate_attributes) == 0x01aa);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, rate_pair_table) == 0x01c0); assert_type_layout!(RatePairTable, 0x40, 4); assert!(core::mem::offset_of!(RatePairTable, pairs) == 0); assert!(core::mem::size_of::<[[SharedU8; 2]; 31]>() == 0x3e); assert!(core::mem::offset_of!(RatePairTable, opaque_3e) == 0x3e); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, rate_pair_table) == 0x0400_01c0); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, rate_pair_table) + core::mem::size_of::<RatePairTable>() == 0x0400_0200); assert_type_layout!(SchedulerTail, 0x60, 4); assert!(core::mem::size_of::<SchedulerTail>() == 0x60); assert!(core::mem::align_of::<SchedulerTail>() == 4);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, initialized_rate_policies) == 0x0200);
    assert!(core::mem::size_of::<InitializedRatePolicies>() == 0x28);
    assert!(core::mem::offset_of!(InitializedRatePolicies, policies) == 0x00);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, completion_callback_prefix) == 0x0228); assert_type_layout!(CompletionCallbackPrefix, 0x38, 4); assert!(core::mem::offset_of!(CompletionCallbackPrefix, completion_prefix_suffix) == 0x00); assert!(core::mem::size_of::<OpaqueBytes<0x30>>() == 0x30); assert!(core::mem::offset_of!(CompletionCallbackPrefix, p2p_action_offsets) == 0x30); assert!(core::mem::size_of::<[SharedU8; 8]>() == 0x08); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, completion_callback_prefix) + core::mem::size_of::<CompletionCallbackPrefix>() == 0x0400_0260);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, visible_completion_words) == 0x0260);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, ring_cursor_map_tables) == 0x0288); assert_type_layout!(RingCursorMapTables, 0x50, 4); assert!(core::mem::offset_of!(RingCursorMapTables, beacon_mask_words) == 0x00); assert!(core::mem::size_of::<[SharedU32; 8]>() == 0x20); assert!(core::mem::offset_of!(RingCursorMapTables, mib_defaults_template) == 0x20); assert!(core::mem::size_of::<OpaqueBytes<0x30>>() == 0x30); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, ring_cursor_map_tables) + core::mem::size_of::<RingCursorMapTables>() == 0x0400_02d8); assert!(core::mem::offset_of!(InitializedDtcmPrefix, queue_pipe_mappings) == 0x02d8);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, pre_tkip_sbox_tables) == 0x02e4);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, tkip_sbox_tables) == 0x0310);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, command_dispatch) == 0x0710);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, debug_console_strings) == 0x07a4); assert_type_layout!(DebugConsoleStrings, 0x60, 4); assert!(core::mem::offset_of!(DebugConsoleStrings, console_help_text) == 0x00); assert!(core::mem::size_of::<OpaqueBytes<0x50>>() == 0x50); assert!(core::mem::offset_of!(DebugConsoleStrings, hex_digits) == 0x50); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, debug_console_strings) + core::mem::size_of::<DebugConsoleStrings>() == 0x0400_0804); assert!(core::mem::offset_of!(InitializedDtcmPrefix, aes_transfer_classes) == 0x0804);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, vendor_encoded_help_text) == 0x0830); assert_type_layout!(VendorEncodedHelpText, 0x1ae, 2); assert!(core::mem::offset_of!(VendorEncodedHelpText, encoded_help_text) == 0); assert!(core::mem::size_of::<OpaqueBytes<0x1ae>>() == 0x1ae);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, vendor_debug_encoded_banner) == 0x09de); assert_type_layout!(VendorDebugEncodedBanner, 0x14a, 2); assert!(core::mem::offset_of!(VendorDebugEncodedBanner, encoded_banner) == 0); assert!(core::mem::size_of::<OpaqueBytes<0x14a>>() == 0x14a); assert!(core::mem::offset_of!(InitializedDtcmPrefix, exception_reason_names) == 0x0b28); assert_type_layout!(ExceptionReasonNames, 0x14, 4); assert!(core::mem::offset_of!(ExceptionReasonNames, names) == 0); assert!(core::mem::size_of::<[SharedU32; 5]>() == 0x14); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, exception_reason_names) == 0x0400_0b28); assert!(core::mem::offset_of!(InitializedDtcmPrefix, hw_timer_debug_tables) == 0x0b3c); assert_type_layout!(HwTimerDebugTables, 0x24, 4); assert!(core::mem::offset_of!(HwTimerDebugTables, divisor_table) == 0x00); assert!(core::mem::size_of::<OpaqueBytes<0x1c>>() == 0x1c); assert!(core::mem::offset_of!(HwTimerDebugTables, channel_config_words) == 0x1c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, hw_timer_debug_tables) + core::mem::size_of::<HwTimerDebugTables>() == 0x0400_0b60);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_gain_register_write_lists) == 0x0b60);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_init_register_write_list_0) == 0x0c10); assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_init_register_write_list_1) == 0x0c20); assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_init_register_write_list_2) == 0x0c48); assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_init_register_write_list_3) == 0x0c60); assert_type_layout!(PhyInitRegisterWriteList3, 0x40, 4); assert!(core::mem::offset_of!(PhyInitRegisterWriteList3, writes) == 0); assert!(core::mem::size_of::<[RegisterWrite; 7]>() == 0x38); assert!(core::mem::offset_of!(PhyInitRegisterWriteList3, terminator_address) == 0x38); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, phy_init_register_write_list_3) + core::mem::size_of::<PhyInitRegisterWriteList3>() == 0x0400_0ca0); assert!(core::mem::offset_of!(InitializedDtcmPrefix, initialized_phy_gain_source_records) == 0x0ca0); assert!(core::mem::size_of::<[InitializedPhyGainSourceRecord; 44]>() == 0x108); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, initialized_phy_gain_source_records) + core::mem::size_of::<[InitializedPhyGainSourceRecord; 44]>() == 0x0400_0da8); assert!(core::mem::offset_of!(InitializedDtcmPrefix, tlv_dispatch_handler_table) == 0x0da8); assert_type_layout!(TlvDispatchHandlerTable, 0x28, 4); assert!(core::mem::offset_of!(TlvDispatchHandlerTable, entries) == 0); assert!(core::mem::size_of::<[TlvDispatchHandlerEntry; 4]>() == 0x20); assert!(core::mem::offset_of!(TlvDispatchHandlerTable, terminator_key) == 0x20); assert!(core::mem::offset_of!(TlvDispatchHandlerTable, terminator_handler) == 0x24); assert_type_layout!(RfModeHalfwordTable, 0x48, 2); assert!(core::mem::offset_of!(RfModeHalfwordTable, entries) == 0); assert!(core::mem::offset_of!(InitializedDtcmPrefix, rf_mode_halfword_table) == 0x0dd0); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, rf_mode_halfword_table) == 0x0400_0dd0); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, rf_mode_halfword_table) + core::mem::size_of::<RfModeHalfwordTable>() == 0x0400_0e18); assert!(RF_MODE_HALFWORD_TABLE.get() + 12 * core::mem::size_of::<SharedU16>() == 0x0400_0de8); assert!(core::mem::offset_of!(InitializedDtcmPrefix, initialized_iq_calibration_gain_indices) == 0x0e18); assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_cal_substate_register_write_list) == 0x0e48); assert_type_layout!(PhyCalSubstateRegisterWriteList, 0x48, 4); assert!(core::mem::offset_of!(PhyCalSubstateRegisterWriteList, writes) == 0); assert!(core::mem::size_of::<[RegisterWrite; 8]>() == 0x40); assert!(core::mem::offset_of!(PhyCalSubstateRegisterWriteList, terminator_address) == 0x40); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, phy_cal_substate_register_write_list) + core::mem::size_of::<PhyCalSubstateRegisterWriteList>() == 0x0400_0e90); assert!(core::mem::offset_of!(InitializedDtcmPrefix, dbg_expand_register_write_list) == 0x0e90); assert_type_layout!(DbgExpandRegisterWriteList, 0x30, 4); assert!(core::mem::offset_of!(DbgExpandRegisterWriteList, writes) == 0); assert!(core::mem::size_of::<[RegisterWrite; 5]>() == 0x28); assert!(core::mem::offset_of!(DbgExpandRegisterWriteList, terminator_address) == 0x28); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, dbg_expand_register_write_list) + core::mem::size_of::<DbgExpandRegisterWriteList>() == 0x0400_0ec0); assert!(core::mem::offset_of!(InitializedDtcmPrefix, pre_rf_scale_tables) == 0x0ec0); assert!(core::mem::size_of::<OpaqueBytes<0xc8>>() == 0xc8); assert_type_layout!(RfScaleHalfwordTable, 0x80, 2); assert!(core::mem::offset_of!(RfScaleHalfwordTable, entries) == 0); assert!(core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_a) == 0x0f88); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_a) + core::mem::size_of::<RfScaleHalfwordTable>() == 0x0400_1008); assert!(core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_b) == 0x1008); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_b) + core::mem::size_of::<RfScaleHalfwordTable>() == 0x0400_1088); assert!(core::mem::offset_of!(InitializedDtcmPrefix, rate_pointer_targets) == 0x1088); assert!(core::mem::size_of::<OpaqueBytes<0x20>>() == 0x20); assert!(core::mem::offset_of!(InitializedDtcmPrefix, tx_gain_rssi_halfword_table) == 0x10a8); assert_type_layout!(TxGainRssiHalfwordTable, 0x2c, 2); assert!(core::mem::offset_of!(TxGainRssiHalfwordTable, entries) == 0); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, tx_gain_rssi_halfword_table) + core::mem::size_of::<TxGainRssiHalfwordTable>() == 0x0400_10d4); assert!(0x14a + 0x14 + 0x24 + 0x0b0 + 0x50 + (0x40 + 0x108 + 0x28 + 0x48 + 0x30 + (0x48 + 0x30 + (0xc8 + 0x80 + 0x80 + 0x4c))) == 0x6f6);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, duration_quantum_pointers) == 0x10d4); assert!(core::mem::offset_of!(InitializedDtcmPrefix, ps_wake_guard_words) == 0x10e4); assert_type_layout!(PsWakeGuardWords, 0x14, 4); assert!(core::mem::offset_of!(PsWakeGuardWords, word_10e4) == 0); assert!(core::mem::size_of::<OpaqueBytes<0x04>>() == 0x04); assert!(core::mem::offset_of!(PsWakeGuardWords, word_10ec) == 0x08); assert!(core::mem::offset_of!(PsWakeGuardWords, word_10f0) == 0x0c); assert!(core::mem::offset_of!(PsWakeGuardWords, opaque_10) == 0x10); assert!(core::mem::offset_of!(InitializedDtcmPrefix, measurement_workspace) == 0x10f8); assert!(core::mem::offset_of!(InitializedDtcmPrefix, tx_aggregate_expiration_delta) == 0x1160); assert!(core::mem::offset_of!(InitializedDtcmPrefix, debug_command_descriptors) == 0x1164);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, hif_control) == 0x11ac);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, irq_callbacks) == 0x11bc); assert_type_layout!(PhyWatchdogCounter, 0x04, 4); assert!(core::mem::offset_of!(PhyWatchdogCounter, count) == 0); assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_watchdog_counter) == 0x123c); assert_type_layout!(InitializedMultiVifBeaconTimerTail, 0x60, 4); assert_type_layout!(SharedU8, 0x01, 1); assert_type_layout!(SharedU32, 0x04, 4); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_00) == 0); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, interface_2_radio_latch) == 0x0f); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, timer) == 0x10); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_24) == 0x24); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_dwell_timer) == 0x3c); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, dtim_capture_latch) == 0x50); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51) == 0x51); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0) == 0x54); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1) == 0x58); assert!(core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2) == 0x5c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_dwell_timer) + core::mem::size_of::<TimerEntry>() == 0x0400_1290); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, dtim_capture_latch) + core::mem::size_of::<SharedU8>() == 0x0400_1291); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51) + core::mem::size_of::<OpaqueBytes<0x03>>() == 0x0400_1294); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0) + core::mem::size_of::<SharedU32>() == 0x0400_1298); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1) + core::mem::size_of::<SharedU32>() == 0x0400_129c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2) + core::mem::size_of::<SharedU32>() == 0x0400_12a0); assert!(core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) == 0x1240); assert!(core::mem::offset_of!(InitializedDtcmPrefix, ampdu_counters) == 0x12a0); assert_type_layout!(InitializedDtcmPrefix, 0x2078, 4); assert_type_layout!(DtcmLayout, DTCM_STATE_SIZE, 4); assert_type_layout!(SharedDtcmState, DTCM_STATE_SIZE, 4);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, ampdu_counters) == 0x12a0); assert_type_layout!(SharedU32, 0x04, 4); assert_type_layout!(PerTidTelemetryBank, 0x140, 4); assert!(core::mem::size_of::<[SharedU32; 8]>() == 0x20);
    assert_type_layout!(RetryPathCounter, 0x04, 4); assert!(core::mem::offset_of!(RetryPathCounter, count) == 0x00); assert!(core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter) == 0x12c8); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter) + core::mem::offset_of!(RetryPathCounter, count) == 0x0400_12c8); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter) + core::mem::size_of::<RetryPathCounter>() == 0x0400_12cc); assert!(core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) == 0x12cc); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_00) == 0x000); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_01) == 0x020); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_02) == 0x040); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_03) == 0x060); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_04) == 0x080); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_05) == 0x0a0); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_06) == 0x0c0); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_07) == 0x0e0); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_08) == 0x100); assert!(core::mem::offset_of!(PerTidTelemetryBank, table_09) == 0x120);
    assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_00) == 0x0400_12cc); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_01) == 0x0400_12ec); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_02) == 0x0400_130c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_03) == 0x0400_132c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_04) == 0x0400_134c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_05) == 0x0400_136c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_06) == 0x0400_138c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_07) == 0x0400_13ac); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_08) == 0x0400_13cc); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::offset_of!(PerTidTelemetryBank, table_09) == 0x0400_13ec); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) + core::mem::size_of::<PerTidTelemetryBank>() == 0x0400_140c); assert!(core::mem::offset_of!(InitializedDtcmPrefix, ampdu_completion_control) == 0x140c);
    assert!(core::mem::size_of::<AmpduCompletionControl>() == 0x04);
    assert!(core::mem::offset_of!(AmpduCompletionControl, enabled) == 0x00);
    assert_type_layout!(SharedU32, 0x04, 4); assert_type_layout!(TxConfirmAggregationState, 0x0c, 4); assert!(core::mem::offset_of!(TxConfirmAggregationState, state) == 0x00); assert!(core::mem::offset_of!(TxConfirmAggregationState, pending_message_raw) == 0x04); assert!(core::mem::offset_of!(TxConfirmAggregationState, append_cursor_raw) == 0x08); assert!(core::mem::offset_of!(InitializedDtcmPrefix, tx_confirm_aggregation_state) == 0x1410); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, tx_confirm_aggregation_state) + core::mem::offset_of!(TxConfirmAggregationState, state) == 0x0400_1410); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, tx_confirm_aggregation_state) + core::mem::offset_of!(TxConfirmAggregationState, pending_message_raw) == 0x0400_1414); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, tx_confirm_aggregation_state) + core::mem::offset_of!(TxConfirmAggregationState, append_cursor_raw) == 0x0400_1418); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, tx_confirm_aggregation_state) + core::mem::size_of::<TxConfirmAggregationState>() == 0x0400_141c); assert_type_layout!(ConfigurationApplyFlags, 0x04, 4); assert!(core::mem::offset_of!(ConfigurationApplyFlags, flags) == 0x00); assert!(core::mem::offset_of!(InitializedDtcmPrefix, configuration_apply_flags) == 0x141c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, configuration_apply_flags) == 0x0400_141c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, configuration_apply_flags) + core::mem::offset_of!(ConfigurationApplyFlags, flags) == 0x0400_141c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, configuration_apply_flags) + core::mem::size_of::<ConfigurationApplyFlags>() == 0x0400_1420); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, control_words) == 0x0400_1420); assert_type_layout!(InitializedDtcmPrefix, 0x2078, 4); assert_type_layout!(DtcmLayout, DTCM_STATE_SIZE, 4); assert_type_layout!(SharedDtcmState, DTCM_STATE_SIZE, 4);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, control_words) == 0x1420);
    assert_type_layout!(SharedU32, 0x04, 4); assert_type_layout!(DebugPlatformLocalTail, 0x08, 4); assert!(core::mem::offset_of!(DebugPlatformLocalTail, platform_local_word_2c) == 0x00); assert!(core::mem::offset_of!(DebugPlatformLocalTail, platform_local_word_30) == 0x04); assert!(core::mem::offset_of!(InitializedDtcmPrefix, pre_phy_channel_threshold_descriptors) == 0x1440); assert!(core::mem::offset_of!(InitializedDtcmPrefix, debug_platform_local_tail) == 0x1454); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, debug_platform_local_tail) + core::mem::offset_of!(DebugPlatformLocalTail, platform_local_word_2c) == 0x0400_1454); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, debug_platform_local_tail) + core::mem::offset_of!(DebugPlatformLocalTail, platform_local_word_30) == 0x0400_1458); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, debug_platform_local_tail) + core::mem::size_of::<DebugPlatformLocalTail>() == 0x0400_145c); assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_channel_threshold_descriptors) == 0x145c); assert_type_layout!(InitializedDtcmPrefix, 0x2078, 4); assert_type_layout!(DtcmLayout, DTCM_STATE_SIZE, 4); assert_type_layout!(SharedDtcmState, DTCM_STATE_SIZE, 4);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_channel_threshold_descriptors) == 0x145c);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, phy_gain_programming_records) == 0x146c);
    ...
    assert_type_layout!(SharedU16, 0x02, 2); assert_type_layout!(PreHostPasRingObserved, 0x0c, 4); assert!(core::mem::offset_of!(PreHostPasRingObserved, opaque_00) == 0x00); assert!(core::mem::offset_of!(PreHostPasRingObserved, radio_stop_word_02) == 0x06); assert!(core::mem::offset_of!(PreHostPasRingObserved, opaque_08) == 0x08); assert!(core::mem::offset_of!(InitializedDtcmPrefix, pre_host_pas_ring) == 0x156c); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, pre_host_pas_ring) + core::mem::offset_of!(PreHostPasRingObserved, radio_stop_word_02) == 0x0400_1572); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, pre_host_pas_ring) + core::mem::offset_of!(PreHostPasRingObserved, radio_stop_word_02) + core::mem::size_of::<SharedU16>() == 0x0400_1574); assert!(core::mem::offset_of!(InitializedDtcmPrefix, host_pas_ring) == 0x1578); assert_type_layout!(InitializedDtcmPrefix, 0x2078, 4); assert_type_layout!(DtcmLayout, DTCM_STATE_SIZE, 4); assert_type_layout!(SharedDtcmState, DTCM_STATE_SIZE, 4);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, initialized_low_mac_prefix) == 0x1680);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, mac_pipe_records) == 0x1720);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, mac_tx_queue_state) == 0x18d0);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, pre_mac_beacon_state) == 0x18d8);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, mac_beacon_state) == 0x1a80);
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, mac_wake_runtime_state) == 0x1ac0);
    ...
    assert_type_layout!(MacAggregateSlotTables, 0x200, 4); assert!(core::mem::offset_of!(MacAggregateSlotTables, queues) == 0); assert!(core::mem::offset_of!(InitializedDtcmPrefix, pre_mac_phy_command_state) == 0x1b08); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, pre_mac_phy_command_state) + core::mem::size_of::<OpaqueBytes<0x08>>() == 0x0400_1b10); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, mac_aggregate_slot_tables) == 0x0400_1b10); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, mac_aggregate_slot_tables) + core::mem::size_of::<MacAggregateSlotTables>() == 0x0400_1d10); assert!(core::mem::offset_of!(InitializedDtcmPrefix, mac_phy_command_state) == 0x1d10);
    ...
    assert!(
        core::mem::offset_of!(InitializedDtcmPrefix, initialized_low_mac_prefix) + 0x7ec == 0x1e6c
    );
    assert!(
        core::mem::offset_of!(InitializedDtcmPrefix, initialized_low_mac_prefix) + 0x8f8 == 0x1f78
    );
    assert!(
        core::mem::offset_of!(InitializedDtcmPrefix, initialized_low_mac_prefix) + 0x940 == 0x1fc0
    );
    ...
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, scheduler_exclusion_state) == 0x1fcc);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, scheduler_event_island) == 0x1fd4);
    assert!(core::mem::offset_of!(InitializedDtcmPrefix, initialized_tail) == 0x2018);
    ...
    assert!(core::mem::offset_of!(SchedulerTail, opaque_204c) == 0x34); assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, initialized_tail) + core::mem::offset_of!(SchedulerTail, opaque_204c) == 0x0400_204c);
    ...
    assert!(core::mem::offset_of!(DtcmLayout, initialized_prefix) == 0x0000);
    ...
    assert!(core::mem::size_of::<InitializedDtcmPrefix>() == DTCM_INITIALIZED_PREFIX_SIZE);
...
mod tests {
    ...
    fn vendor_bss_word_addresses_are_exact() {
        ...
        assert_eq!(DTCM_RUNTIME_END.get(), 0x0400_9c44);
        assert_eq!(DTCM_RUNTIME_SUFFIX_SIZE / core::mem::size_of::<SharedU32>(), 0x1ef3);
        assert_eq!(VENDOR_BSS_START.get() + DTCM_RUNTIME_SUFFIX_SIZE, DTCM_RUNTIME_END.get());
    }
    ...
    fn initialized_mac_wake_runtime_addresses_are_exact() {
        ...
        assert_eq!(mac_secondary_rate_overlay_unchecked(14).get(), 0x0400_1b08);
        assert_eq!(mac_secondary_rate_overlay_unchecked(21).get(), 0x0400_1b0f);
    ...
    fn initialized_low_mac_global_addresses_are_exact() {
        ...
        assert_eq!(LOW_MAC_FIFO_CONTROL.get(), 0x0400_1680);
        assert_eq!(LOW_MAC_FIFO_STATUS.get(), 0x0400_1681);
        assert_eq!(LOW_MAC_RATE_CONFIG.get(), 0x0400_1682);
        assert_eq!(LOW_MAC_CONTROL_04.get(), 0x0400_1684);
        ...
        assert_eq!(LOW_MAC_SLOT_TIME_X2.get(), 0x0400_16a8);
        assert_eq!(LOW_MAC_SLOT_TIME_X3.get(), 0x0400_16ac);
        assert_eq!(LOW_MAC_SLOT_TIME_CONSTANT.get(), 0x0400_16b0);
    ...
    fn initialized_mac_pipe_record_addresses_are_exact() {
        ...
        let first = MacPipeRecordAddress::from_index(0).unwrap();
        let last = MacPipeRecordAddress::from_index(3).unwrap();
        let first_slot = first.slot_unchecked(0);
        assert_eq!(first.raw(), 0x0400_1720);
        assert_eq!(first.producer_slot().get(), 0x0400_1720);
        assert_eq!(first.last_slot().get(), 0x0400_1721);
        assert_eq!(first.current_slot().get(), 0x0400_1722);
        assert_eq!(first.state().get(), 0x0400_1723);
        assert_eq!(first.control().get(), 0x0400_1724);
        assert_eq!(first.watchdog().get(), 0x0400_1725);
        assert_eq!(first.abort_status().get(), 0x0400_1726);
    ...
    #[test]
    fn initialized_pre_host_pas_radio_stop_word_address_is_exact() { let image = DTCM_STATE_BASE; let observed = image + core::mem::offset_of!(InitializedDtcmPrefix, pre_host_pas_ring); let prefix = observed + core::mem::offset_of!(PreHostPasRingObserved, opaque_00); let word = observed + core::mem::offset_of!(PreHostPasRingObserved, radio_stop_word_02); let suffix = observed + core::mem::offset_of!(PreHostPasRingObserved, opaque_08); assert_eq!(core::mem::size_of::<SharedU16>(), 0x02); assert_eq!(core::mem::align_of::<SharedU16>(), 2); assert_eq!([core::mem::offset_of!(PreHostPasRingObserved, opaque_00), core::mem::offset_of!(PreHostPasRingObserved, radio_stop_word_02), core::mem::offset_of!(PreHostPasRingObserved, opaque_08)], [0x00, 0x06, 0x08]); assert_eq!(core::mem::size_of::<PreHostPasRingObserved>(), 0x0c); assert_eq!(core::mem::align_of::<PreHostPasRingObserved>(), 4); assert_eq!(observed, 0x0400_156c); assert_eq!(prefix, 0x0400_156c); assert_eq!(core::mem::size_of::<OpaqueBytes<0x06>>(), 0x06); assert_eq!(prefix + core::mem::size_of::<OpaqueBytes<0x06>>(), 0x0400_1572); assert_eq!(RADIO_STOP_WORD_02.get(), 0x0400_1572); assert_eq!(word, RADIO_STOP_WORD_02.get()); assert_eq!(word + core::mem::size_of::<SharedU16>(), 0x0400_1574); assert_eq!(suffix, 0x0400_1574); assert_eq!(core::mem::size_of::<OpaqueBytes<0x04>>(), 0x04); assert_eq!(suffix + core::mem::size_of::<OpaqueBytes<0x04>>(), 0x0400_1578); assert_eq!(HOST_PAS_RING.get(), 0x0400_1578); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); }
    ...
    #[test]
    fn initialized_phy_watchdog_counter_address_is_exact() { assert_eq!(core::mem::size_of::<PhyWatchdogCounter>(), 0x04); assert_eq!(core::mem::align_of::<PhyWatchdogCounter>(), 4); assert_eq!(core::mem::offset_of!(PhyWatchdogCounter, count), 0); assert_eq!(IRQ_CALLBACK_TABLE.get() + core::mem::size_of::<[SharedU32; 32]>(), 0x0400_123c); assert_eq!(PHY_WATCHDOG_COUNTER.get(), 0x0400_123c); assert_eq!(PHY_WATCHDOG_COUNTER.get() + core::mem::size_of::<PhyWatchdogCounter>(), 0x0400_1240); assert_eq!(core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail), 0x1240); assert_eq!(core::mem::size_of::<InitializedMultiVifBeaconTimerTail>(), 0x60); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::size_of::<InitializedMultiVifBeaconTimerTail>(), 0x0400_12a0); assert_eq!(AMPDU_TELEMETRY_COUNTERS.get(), 0x0400_12a0); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test] fn initialized_multi_vif_beacon_timer_address_is_exact() { assert_eq!(core::mem::size_of::<TimerEntry>(), 0x14); assert_eq!(core::mem::align_of::<TimerEntry>(), 4); assert_eq!([core::mem::offset_of!(TimerEntry, next), core::mem::offset_of!(TimerEntry, previous_link), core::mem::offset_of!(TimerEntry, deadline), core::mem::offset_of!(TimerEntry, callback), core::mem::offset_of!(TimerEntry, context)], [0x00, 0x04, 0x08, 0x0c, 0x10]); assert_eq!(core::mem::size_of::<InitializedMultiVifBeaconTimerTail>(), 0x60); assert_eq!(core::mem::align_of::<InitializedMultiVifBeaconTimerTail>(), 4); assert_eq!([core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_00), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, interface_2_radio_latch), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, timer), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_24), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_dwell_timer), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, dtim_capture_latch), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2)], [0x00, 0x0f, 0x10, 0x24, 0x3c, 0x50, 0x51, 0x54, 0x58, 0x5c]); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail), 0x0400_1240); assert_eq!(MULTI_VIF_BEACON_TIMER.get(), 0x0400_1250); assert_eq!(MULTI_VIF_BEACON_TIMER.get() + core::mem::size_of::<TimerEntry>(), 0x0400_1264); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51) + core::mem::size_of::<OpaqueBytes<0x03>>(), 0x0400_1294); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0) + core::mem::size_of::<SharedU32>(), 0x0400_1298); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1) + core::mem::size_of::<SharedU32>(), 0x0400_129c); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail) + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2) + core::mem::size_of::<SharedU32>(), 0x0400_12a0); assert_eq!(AMPDU_TELEMETRY_COUNTERS.get(), 0x0400_12a0); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test] fn initialized_measurement_dwell_timer_address_is_exact() { assert_eq!([core::mem::offset_of!(TimerEntry, next), core::mem::offset_of!(TimerEntry, previous_link), core::mem::offset_of!(TimerEntry, deadline), core::mem::offset_of!(TimerEntry, callback), core::mem::offset_of!(TimerEntry, context)], [0x00, 0x04, 0x08, 0x0c, 0x10]); assert_eq!(core::mem::size_of::<TimerEntry>(), 0x14); assert_eq!(core::mem::align_of::<TimerEntry>(), 4); assert_eq!([core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_00), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, interface_2_radio_latch), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, timer), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_24), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_dwell_timer), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, dtim_capture_latch), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2)], [0x00, 0x0f, 0x10, 0x24, 0x3c, 0x50, 0x51, 0x54, 0x58, 0x5c]); assert_eq!(core::mem::size_of::<InitializedMultiVifBeaconTimerTail>(), 0x60); assert_eq!(core::mem::align_of::<InitializedMultiVifBeaconTimerTail>(), 4); let tail = DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail); assert_eq!(tail, 0x0400_1240); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_24) + core::mem::size_of::<OpaqueBytes<0x18>>(), 0x0400_127c); assert_eq!(MEASUREMENT_DWELL_TIMER.get(), 0x0400_127c); assert_eq!(MEASUREMENT_DWELL_TIMER.get() + core::mem::size_of::<TimerEntry>(), 0x0400_1290); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51) + core::mem::size_of::<OpaqueBytes<0x03>>(), 0x0400_1294); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0) + core::mem::size_of::<SharedU32>(), 0x0400_1298); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1) + core::mem::size_of::<SharedU32>(), 0x0400_129c); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2) + core::mem::size_of::<SharedU32>(), 0x0400_12a0); assert_eq!(AMPDU_TELEMETRY_COUNTERS.get(), 0x0400_12a0); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test] fn initialized_dtim_capture_latch_address_is_exact() { let tail = DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail); let latch = tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, dtim_capture_latch); let opaque = tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51); assert_eq!(tail, 0x0400_1240); assert_eq!(MEASUREMENT_DWELL_TIMER.get() + core::mem::size_of::<TimerEntry>(), 0x0400_1290); assert_eq!([core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, dtim_capture_latch), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2)], [0x50, 0x51, 0x54, 0x58, 0x5c]); assert_eq!(core::mem::size_of::<SharedU8>(), 0x01); assert_eq!(core::mem::align_of::<SharedU8>(), 1); assert_eq!(latch, 0x0400_1290); assert_eq!(latch + core::mem::size_of::<SharedU8>(), 0x0400_1291); assert_eq!(core::mem::size_of::<OpaqueBytes<0x03>>(), 0x03); assert_eq!(opaque, 0x0400_1291); assert_eq!(opaque + core::mem::size_of::<OpaqueBytes<0x03>>(), 0x0400_1294); assert_eq!([tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0), tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1), tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2)], [0x0400_1294, 0x0400_1298, 0x0400_129c]); assert_eq!(tail + core::mem::size_of::<InitializedMultiVifBeaconTimerTail>(), 0x0400_12a0); assert_eq!(AMPDU_TELEMETRY_COUNTERS.get(), 0x0400_12a0); assert_eq!(core::mem::size_of::<InitializedMultiVifBeaconTimerTail>(), 0x60); assert_eq!(core::mem::align_of::<InitializedMultiVifBeaconTimerTail>(), 4); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test] fn initialized_measurement_control_reset_words_are_exact() { let tail = DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail); assert_eq!(tail, 0x0400_1240); assert_eq!(MEASUREMENT_DWELL_TIMER.get() + core::mem::size_of::<TimerEntry>(), 0x0400_1290); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, dtim_capture_latch), 0x0400_1290); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51) + core::mem::size_of::<OpaqueBytes<0x03>>(), 0x0400_1294); assert_eq!([tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0), tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1), tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2)], [0x0400_1294, 0x0400_1298, 0x0400_129c]); assert_eq!(core::mem::size_of::<SharedU32>(), 0x04); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2) + core::mem::size_of::<SharedU32>(), 0x0400_12a0); assert_eq!(AMPDU_TELEMETRY_COUNTERS.get(), 0x0400_12a0); assert_eq!(core::mem::size_of::<InitializedMultiVifBeaconTimerTail>(), 0x60); assert_eq!(core::mem::align_of::<InitializedMultiVifBeaconTimerTail>(), 4); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test] fn initialized_mac_aggregate_slot_tables_are_exact() { let image = DTCM_STATE_BASE; let prefix = image + core::mem::offset_of!(InitializedDtcmPrefix, pre_mac_phy_command_state); let tables = image + core::mem::offset_of!(InitializedDtcmPrefix, mac_aggregate_slot_tables); assert_eq!(core::mem::size_of::<MacAggregateSlotTables>(), 0x200); assert_eq!(core::mem::align_of::<MacAggregateSlotTables>(), 4); assert_eq!(core::mem::offset_of!(MacAggregateSlotTables, queues), 0); assert_eq!(prefix, 0x0400_1b08); assert_eq!(core::mem::size_of::<OpaqueBytes<0x08>>(), 0x08); assert_eq!(tables, 0x0400_1b10); assert_eq!(tables + 7 * 0x40, 0x0400_1cd0); assert_eq!(tables + 7 * 0x40 + 15 * core::mem::size_of::<SharedU32>(), 0x0400_1d0c); assert_eq!(tables + core::mem::size_of::<MacAggregateSlotTables>(), 0x0400_1d10); assert_eq!(MAC_PHY_COMMAND_STATE.get(), 0x0400_1d10); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test] fn initialized_interface_2_radio_latch_address_is_exact() { let tail = DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail); let latch = tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, interface_2_radio_latch); let timer = tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, timer); assert_eq!(tail, 0x0400_1240); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_00), 0x0400_1240); assert_eq!(tail + core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_00) + core::mem::size_of::<OpaqueBytes<0x0f>>(), 0x0400_124f); assert_eq!(latch, 0x0400_124f); assert_eq!(latch + core::mem::size_of::<SharedU8>(), 0x0400_1250); assert_eq!(timer, 0x0400_1250); assert_eq!(timer + core::mem::size_of::<TimerEntry>(), 0x0400_1264); assert_eq!([core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_00), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, interface_2_radio_latch), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, timer), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_24), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_dwell_timer), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, dtim_capture_latch), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, opaque_51), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_0), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_1), core::mem::offset_of!(InitializedMultiVifBeaconTimerTail, measurement_control_word_2)], [0x00, 0x0f, 0x10, 0x24, 0x3c, 0x50, 0x51, 0x54, 0x58, 0x5c]); assert_eq!(core::mem::size_of::<SharedU8>(), 0x01); assert_eq!(core::mem::align_of::<SharedU8>(), 1); assert_eq!(core::mem::size_of::<InitializedMultiVifBeaconTimerTail>(), 0x60); assert_eq!(core::mem::align_of::<InitializedMultiVifBeaconTimerTail>(), 4); assert_eq!(core::mem::offset_of!(InitializedDtcmPrefix, multi_vif_beacon_timer_tail), 0x1240); assert_eq!(core::mem::offset_of!(InitializedDtcmPrefix, ampdu_counters), 0x12a0); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); }
    ...
    #[test]
    fn initialized_scheduler_tail_is_exact() { let image = DTCM_STATE_BASE; let tail = image + core::mem::offset_of!(InitializedDtcmPrefix, initialized_tail); assert_eq!(core::mem::size_of::<SchedulerTail>(), 0x60); assert_eq!(core::mem::align_of::<SchedulerTail>(), 4); assert_eq!(core::mem::offset_of!(SchedulerTail, hardware_timer_guard), 0x10); assert_eq!(tail + core::mem::offset_of!(SchedulerTail, hardware_timer_guard), 0x0400_2028); assert_eq!(SCHEDULER_HARDWARE_TIMER_GUARD.get(), 0x0400_2028); assert_eq!(core::mem::offset_of!(SchedulerTail, rf_calibration_bytes), 0x14); assert_eq!(core::mem::size_of::<[SharedU8; 32]>(), 0x20); assert_eq!(tail + core::mem::offset_of!(SchedulerTail, rf_calibration_bytes), 0x0400_202c); assert_eq!(core::mem::offset_of!(SchedulerTail, error_event_counts), 0x38); assert_eq!(core::mem::size_of::<[SharedU32; 10]>(), 0x28); assert_eq!(tail + core::mem::offset_of!(SchedulerTail, error_event_counts), 0x0400_2050); assert_eq!(tail + core::mem::offset_of!(SchedulerTail, error_event_counts) + core::mem::size_of::<[SharedU32; 10]>(), 0x0400_2078); assert_eq!(tail + core::mem::size_of::<SchedulerTail>(), 0x0400_2078); assert_eq!(scheduler_timer_list_head().get(), 0x0400_2014); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test]
    fn register_write_lists_after_iq_gain_indices_are_exact() { let image = DTCM_STATE_BASE; let cal = image + core::mem::offset_of!(InitializedDtcmPrefix, phy_cal_substate_register_write_list); let dbg = image + core::mem::offset_of!(InitializedDtcmPrefix, dbg_expand_register_write_list); assert_eq!(core::mem::size_of::<PhyCalSubstateRegisterWriteList>(), 0x48); assert_eq!(core::mem::align_of::<PhyCalSubstateRegisterWriteList>(), 4); assert_eq!(core::mem::offset_of!(PhyCalSubstateRegisterWriteList, writes), 0); assert_eq!(core::mem::size_of::<[RegisterWrite; 8]>(), 0x40); assert_eq!(core::mem::offset_of!(PhyCalSubstateRegisterWriteList, terminator_address), 0x40); assert_eq!(cal, 0x0400_0e48); assert_eq!(cal + core::mem::size_of::<PhyCalSubstateRegisterWriteList>(), 0x0400_0e90); assert_eq!(core::mem::size_of::<DbgExpandRegisterWriteList>(), 0x30); assert_eq!(core::mem::align_of::<DbgExpandRegisterWriteList>(), 4); assert_eq!(core::mem::offset_of!(DbgExpandRegisterWriteList, writes), 0); assert_eq!(core::mem::size_of::<[RegisterWrite; 5]>(), 0x28); assert_eq!(core::mem::offset_of!(DbgExpandRegisterWriteList, terminator_address), 0x28); assert_eq!(dbg, 0x0400_0e90); assert_eq!(dbg + core::mem::size_of::<DbgExpandRegisterWriteList>(), 0x0400_0ec0); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, pre_rf_scale_tables), 0x0400_0ec0); assert_eq!(core::mem::size_of::<OpaqueBytes<0xc8>>(), 0xc8); assert_eq!(core::mem::size_of::<RfScaleHalfwordTable>(), 0x80); assert_eq!(core::mem::align_of::<RfScaleHalfwordTable>(), 2); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_a), 0x0400_0f88); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_a) + core::mem::size_of::<RfScaleHalfwordTable>(), 0x0400_1008); assert_eq!(RF_SCALE_TABLE_A_MODE0_TARGET.get(), 0x0400_1000); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_b), 0x0400_1008); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, rf_scale_table_b) + core::mem::size_of::<RfScaleHalfwordTable>(), 0x0400_1088); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, rate_pointer_targets), 0x0400_1088); assert_eq!(PHY_RATE_POINTER_TARGET_A.get(), 0x0400_1088); assert_eq!(PHY_RATE_POINTER_TARGET_B.get(), 0x0400_1098); assert_eq!(core::mem::size_of::<OpaqueBytes<0x20>>(), 0x20); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, tx_gain_rssi_halfword_table), 0x0400_10a8); assert_eq!(core::mem::size_of::<TxGainRssiHalfwordTable>(), 0x2c); assert_eq!(core::mem::align_of::<TxGainRssiHalfwordTable>(), 2); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, tx_gain_rssi_halfword_table) + core::mem::size_of::<TxGainRssiHalfwordTable>(), 0x0400_10d4); assert_eq!(DURATION_QUANTUM_POINTERS.get(), 0x0400_10d4); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test]
    fn initialized_rate_pair_table_is_exact() { let image = DTCM_STATE_BASE; let table = image + core::mem::offset_of!(InitializedDtcmPrefix, rate_pair_table); assert_eq!(core::mem::size_of::<RatePairTable>(), 0x40); assert_eq!(core::mem::align_of::<RatePairTable>(), 4); assert_eq!(core::mem::offset_of!(RatePairTable, pairs), 0); assert_eq!(core::mem::size_of::<[[SharedU8; 2]; 31]>(), 0x3e); assert_eq!(core::mem::offset_of!(RatePairTable, opaque_3e), 0x3e); assert_eq!(core::mem::size_of::<OpaqueBytes<0x02>>(), 0x02); assert_eq!(table, 0x0400_01c0); assert_eq!(table + core::mem::size_of::<RatePairTable>(), 0x0400_0200); assert_eq!(INITIALIZED_RATE_POLICIES.get(), 0x0400_0200); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test]
    ...
    fn initialized_tkip_sbox_table_addresses_are_exact() {
        ...
        assert_eq!(TKIP_SBOX_TABLES.get() + core::mem::size_of::<TkipSboxTables>(), DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, command_dispatch));
    }
    ...
    fn initialized_aes_transfer_class_addresses_are_exact() {
        ...
        assert_eq!(AES_TRANSFER_CLASSES.get() + core::mem::size_of::<AesTransferClassTable>(), DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, vendor_encoded_help_text));
    }
    ...
    #[test]
    fn vendor_debug_tables_are_exact() {
        ...
        assert_eq!(tables, 0x0400_0b3c); assert_eq!(tables + core::mem::offset_of!(HwTimerDebugTables, channel_config_words), 0x0400_0b58);
    ...
    fn initialized_phy_init_register_write_list_addresses_are_exact() {
        ...
        assert_eq!([phy_init_register_write_list(0).unwrap().get(), phy_init_register_write_list(1).unwrap().get(), phy_init_register_write_list(2).unwrap().get(), phy_init_register_write_list(3).unwrap().get()], [0x0400_0c10, 0x0400_0c20, 0x0400_0c48, 0x0400_0c60]);
        ...
        assert_eq!([phy_init_register_write_entry(2, 0).unwrap().get(), phy_init_register_write_entry(2, 1).unwrap().get()], [0x0400_0c48, 0x0400_0c50]);
         assert!(phy_init_register_write_entry(0, 1).is_none()); assert!(phy_init_register_write_entry(1, 4).is_none()); assert!(phy_init_register_write_entry(2, 2).is_none());
        ...
        assert_eq!(phy_init_register_write_list(2).unwrap().get() + core::mem::size_of::<PhyInitRegisterWriteList2>(), 0x0400_0c60);
        assert_eq!([phy_init_register_write_entry(3, 0).unwrap().get(), phy_init_register_write_entry(3, 6).unwrap().get()], [0x0400_0c60, 0x0400_0c90]);
        assert_eq!(phy_init_register_write_list(3).unwrap().get() + core::mem::size_of::<PhyInitRegisterWriteList3>(), 0x0400_0ca0); assert!(phy_init_register_write_entry(3, 7).is_none()); assert_eq!(DURATION_QUANTUM_POINTERS.get(), 0x0400_10d4);
    }

    #[test]
fn initialized_phy_gain_source_record_addresses_are_exact() { assert_eq!(INITIALIZED_PHY_GAIN_SOURCE_RECORDS.get(), 0x0400_0ca0); assert_eq!(initialized_phy_gain_source_physical_record(0).unwrap().get(), 0x0400_0ca0); assert_eq!(initialized_phy_gain_source_physical_record(21).unwrap().get(), 0x0400_0d1e); assert_eq!(initialized_phy_gain_source_physical_record(22).unwrap().get(), 0x0400_0d24); assert_eq!(initialized_phy_gain_source_physical_record(43).unwrap().get(), 0x0400_0da2); assert_eq!(initialized_phy_gain_source_physical_record(43).unwrap().get() + core::mem::size_of::<InitializedPhyGainSourceRecord>(), 0x0400_0da8); assert!(initialized_phy_gain_source_physical_record(44).is_none()); assert_eq!(initialized_phy_gain_source_view_record(0, 0).unwrap().get(), 0x0400_0ca0); assert_eq!(initialized_phy_gain_source_view_record(0, 21).unwrap().get(), 0x0400_0d1e); assert_eq!(initialized_phy_gain_source_view_record(1, 0).unwrap().get(), 0x0400_0d24); assert_eq!(initialized_phy_gain_source_view_record(1, 21).unwrap().get(), 0x0400_0da2); assert!(initialized_phy_gain_source_view_record(2, 0).is_none()); assert!(initialized_phy_gain_source_view_record(0, 22).is_none()); assert_eq!(core::mem::size_of::<InitializedPhyGainSourceRecord>(), 0x06); assert_eq!(core::mem::align_of::<InitializedPhyGainSourceRecord>(), 2); assert_eq!(core::mem::size_of::<[InitializedPhyGainSourceRecord; 44]>(), 0x108); assert_eq!(core::mem::align_of::<[InitializedPhyGainSourceRecord; 44]>(), 2); assert_eq!([core::mem::offset_of!(InitializedPhyGainSourceRecord, selector), core::mem::offset_of!(InitializedPhyGainSourceRecord, opaque_01), core::mem::offset_of!(InitializedPhyGainSourceRecord, lower), core::mem::offset_of!(InitializedPhyGainSourceRecord, upper)], [0, 1, 2, 4]); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(DURATION_QUANTUM_POINTERS.get(), 0x0400_10d4); }    #[test]
    ...
    #[test]
    fn initialized_prefix_tables_are_exact() { let image = DTCM_STATE_BASE; let list = image + core::mem::offset_of!(InitializedDtcmPrefix, mac_slot_timing_patch_list); let quanta = image + core::mem::offset_of!(InitializedDtcmPrefix, pac_duration_quanta); let suffix = image + core::mem::offset_of!(InitializedDtcmPrefix, prefix_suffix); assert_eq!(list, DTCM_STATE_BASE); assert_eq!(core::mem::size_of::<MacSlotTimingPatchEntry>(), 0x08); assert_eq!([core::mem::offset_of!(MacSlotTimingPatchEntry, pointer), core::mem::offset_of!(MacSlotTimingPatchEntry, patch_word)], [0x00, 0x04]); assert_eq!(core::mem::size_of::<MacSlotTimingPatchList>(), 0x58); assert_eq!(core::mem::size_of::<[MacSlotTimingPatchEntry; 11]>(), 0x58); assert_eq!(list + core::mem::size_of::<MacSlotTimingPatchList>(), 0x0400_0058); assert_eq!(quanta, 0x0400_0058); assert_eq!(core::mem::size_of::<PacDurationQuanta>(), 0x20); assert_eq!(quanta + core::mem::size_of::<PacDurationQuanta>(), 0x0400_0078); assert_eq!(suffix, 0x0400_0078); assert_eq!(core::mem::size_of::<OpaqueBytes<0xc0>>(), 0xc0); assert_eq!(suffix + core::mem::size_of::<OpaqueBytes<0xc0>>(), 0x0400_0138); assert_eq!(TX_DURATION_TIMING_TABLE.get(), 0x0400_0138); let pas_tables = image + core::mem::offset_of!(InitializedDtcmPrefix, pas_rate_static_tables); assert_eq!(pas_tables, 0x0400_014c); assert_eq!(core::mem::size_of::<PasRateStaticTables>(), 0x48); assert_eq!([core::mem::offset_of!(PasRateStaticTables, mcs_fallback_rates), core::mem::offset_of!(PasRateStaticTables, pas_fallback_suffix), core::mem::offset_of!(PasRateStaticTables, ofdm_airtime_durations), core::mem::offset_of!(PasRateStaticTables, rate_group_records)], [0x00, 0x08, 0x10, 0x30]); assert_eq!(pas_tables + core::mem::size_of::<[SharedU8; 8]>(), 0x0400_0154); assert_eq!(pas_tables + core::mem::offset_of!(PasRateStaticTables, ofdm_airtime_durations), 0x0400_015c); assert_eq!(pas_tables + core::mem::offset_of!(PasRateStaticTables, rate_group_records), 0x0400_017c); assert_eq!(pas_tables + core::mem::size_of::<PasRateStaticTables>(), 0x0400_0194); let ccb = image + core::mem::offset_of!(InitializedDtcmPrefix, completion_callback_prefix); assert_eq!(ccb, 0x0400_0228); assert_eq!(core::mem::size_of::<CompletionCallbackPrefix>(), 0x38); assert_eq!([core::mem::offset_of!(CompletionCallbackPrefix, completion_prefix_suffix), core::mem::offset_of!(CompletionCallbackPrefix, p2p_action_offsets)], [0x00, 0x30]); assert_eq!(ccb + core::mem::size_of::<OpaqueBytes<0x30>>(), 0x0400_0258); assert_eq!(ccb + core::mem::size_of::<CompletionCallbackPrefix>(), 0x0400_0260);
        let rcm = image + core::mem::offset_of!(InitializedDtcmPrefix, ring_cursor_map_tables); assert_eq!(rcm, 0x0400_0288); assert_eq!(core::mem::size_of::<RingCursorMapTables>(), 0x50); assert_eq!([core::mem::offset_of!(RingCursorMapTables, beacon_mask_words), core::mem::offset_of!(RingCursorMapTables, mib_defaults_template)], [0x00, 0x20]); assert_eq!(rcm + core::mem::size_of::<[SharedU32; 8]>(), 0x0400_02a8); assert_eq!(rcm + core::mem::size_of::<RingCursorMapTables>(), 0x0400_02d8); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test]
    fn tlv_dispatch_handler_table_is_exact() { let image = DTCM_STATE_BASE; let tlv = image + core::mem::offset_of!(InitializedDtcmPrefix, tlv_dispatch_handler_table); assert_eq!(core::mem::size_of::<TlvDispatchHandlerTable>(), 0x28); assert_eq!(core::mem::align_of::<TlvDispatchHandlerTable>(), 4); assert_eq!(core::mem::offset_of!(TlvDispatchHandlerTable, entries), 0); assert_eq!(core::mem::size_of::<[TlvDispatchHandlerEntry; 4]>(), 0x20); assert_eq!([core::mem::offset_of!(TlvDispatchHandlerTable, terminator_key), core::mem::offset_of!(TlvDispatchHandlerTable, terminator_handler)], [0x20, 0x24]); assert_eq!(tlv, 0x0400_0da8); assert_eq!(tlv + core::mem::offset_of!(TlvDispatchHandlerTable, terminator_key), 0x0400_0dc8); assert_eq!(tlv + core::mem::size_of::<TlvDispatchHandlerTable>(), 0x0400_0dd0); assert_eq!(RF_MODE_HALFWORD_TABLE.get(), 0x0400_0dd0); assert_eq!(INITIALIZED_IQ_CALIBRATION_GAIN_INDICES.get(), 0x0400_0e18); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test]
    fn initialized_rf_mode_halfword_table_is_exact() { let image = DTCM_STATE_BASE; let table = image + core::mem::offset_of!(InitializedDtcmPrefix, rf_mode_halfword_table); let tlv = image + core::mem::offset_of!(InitializedDtcmPrefix, tlv_dispatch_handler_table); assert_eq!(tlv, 0x0400_0da8); assert_eq!(core::mem::size_of::<TlvDispatchHandlerTable>(), 0x28); assert_eq!(core::mem::align_of::<TlvDispatchHandlerTable>(), 4); assert_eq!(core::mem::offset_of!(TlvDispatchHandlerTable, entries), 0); assert_eq!(core::mem::size_of::<[TlvDispatchHandlerEntry; 4]>(), 0x20); assert_eq!([core::mem::offset_of!(TlvDispatchHandlerTable, terminator_key), core::mem::offset_of!(TlvDispatchHandlerTable, terminator_handler)], [0x20, 0x24]); assert_eq!(table, 0x0400_0dd0); assert_eq!(core::mem::align_of::<RfModeHalfwordTable>(), 2); assert_eq!(core::mem::offset_of!(RfModeHalfwordTable, entries), 0); assert_eq!(core::mem::size_of::<RfModeHalfwordTable>(), 0x48); assert_eq!([RF_MODE_HALFWORD_TABLE.get() + 0 * 2, RF_MODE_HALFWORD_TABLE.get() + 12 * 2, RF_MODE_HALFWORD_TABLE.get() + 35 * 2], [0x0400_0dd0, 0x0400_0de8, 0x0400_0e16]); assert_eq!(table + core::mem::size_of::<RfModeHalfwordTable>(), 0x0400_0e18); assert_eq!(INITIALIZED_IQ_CALIBRATION_GAIN_INDICES.get(), 0x0400_0e18); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test]
    fn initialized_iq_calibration_gain_index_addresses_are_exact() { assert_eq!(core::mem::size_of::<InitializedIqCalibrationGainIndices>(), 0x30); assert_eq!(core::mem::align_of::<InitializedIqCalibrationGainIndices>(), 4); assert_eq!(core::mem::offset_of!(InitializedIqCalibrationGainIndices, entries), 0); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, tlv_dispatch_handler_table), 0x0400_0da8); assert_eq!(core::mem::size_of::<TlvDispatchHandlerTable>(), 0x28); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, rf_mode_halfword_table), 0x0400_0dd0); assert_eq!(core::mem::size_of::<RfModeHalfwordTable>(), 0x48); assert_eq!(RF_MODE_HALFWORD_TABLE.get(), 0x0400_0dd0); assert_eq!(RF_MODE_HALFWORD_TABLE.get() + core::mem::size_of::<RfModeHalfwordTable>(), 0x0400_0e18); assert_eq!(INITIALIZED_IQ_CALIBRATION_GAIN_INDICES.get(), 0x0400_0e18); assert_eq!(initialized_iq_calibration_gain_index(0).unwrap().get(), 0x0400_0e18); assert_eq!(initialized_iq_calibration_gain_index(1).unwrap().get(), 0x0400_0e1c); assert_eq!(initialized_iq_calibration_gain_index(11).unwrap().get(), 0x0400_0e44); assert!(initialized_iq_calibration_gain_index(12).is_none()); assert_eq!(INITIALIZED_IQ_CALIBRATION_GAIN_INDICES.get() + core::mem::size_of::<InitializedIqCalibrationGainIndices>(), 0x0400_0e48); assert_eq!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, phy_cal_substate_register_write_list), 0x0400_0e48); assert_eq!(core::mem::size_of::<PhyCalSubstateRegisterWriteList>(), 0x48); assert_eq!(core::mem::size_of::<OpaqueBytes<0x214>>(), 0x214); assert_eq!(DURATION_QUANTUM_POINTERS.get(), 0x0400_10d4); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); }
    #[test]
    fn ps_wake_guard_words_are_exact() { let image = DTCM_STATE_BASE; let record = image + core::mem::offset_of!(InitializedDtcmPrefix, ps_wake_guard_words); assert_eq!(core::mem::size_of::<PsWakeGuardWords>(), 0x14); assert_eq!(core::mem::align_of::<PsWakeGuardWords>(), 4); assert_eq!([core::mem::offset_of!(PsWakeGuardWords, word_10e4), core::mem::offset_of!(PsWakeGuardWords, word_10ec), core::mem::offset_of!(PsWakeGuardWords, word_10f0), core::mem::offset_of!(PsWakeGuardWords, opaque_10)], [0x00, 0x08, 0x0c, 0x10]); assert_eq!(record, 0x0400_10e4); assert_eq!(record + core::mem::offset_of!(PsWakeGuardWords, word_10ec), 0x0400_10ec); assert_eq!(record + core::mem::offset_of!(PsWakeGuardWords, word_10f0), 0x0400_10f0); assert_eq!(record + core::mem::size_of::<PsWakeGuardWords>(), 0x0400_10f8); assert_eq!(DURATION_QUANTUM_POINTERS.get() + 0x10, 0x0400_10e4); assert_eq!(MEASUREMENT_WORKSPACE.get(), 0x0400_10f8); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test]
    ...
    #[test]
    fn initialized_retry_path_counter_address_is_exact() { let image = DTCM_STATE_BASE; let record = image + core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter); let field = record + core::mem::offset_of!(RetryPathCounter, count); assert_eq!(core::mem::size_of::<SharedU32>(), 0x04); assert_eq!(core::mem::align_of::<SharedU32>(), 4); assert_eq!(core::mem::size_of::<RetryPathCounter>(), 0x04); assert_eq!(core::mem::align_of::<RetryPathCounter>(), 4); assert_eq!(core::mem::offset_of!(RetryPathCounter, count), 0x00); assert_eq!(AMPDU_TELEMETRY_COUNTERS.get() + core::mem::size_of::<AmpduTelemetryCounters>(), 0x0400_12c8); assert_eq!(record, 0x0400_12c8); assert_eq!(field, 0x0400_12c8); assert_eq!(record + core::mem::size_of::<RetryPathCounter>(), 0x0400_12cc); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank), 0x0400_12cc); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test] fn initialized_per_tid_telemetry_bank_addresses_are_exact() { let image = DTCM_STATE_BASE; let bank = image + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank); let roots = [bank + core::mem::offset_of!(PerTidTelemetryBank, table_00), bank + core::mem::offset_of!(PerTidTelemetryBank, table_01), bank + core::mem::offset_of!(PerTidTelemetryBank, table_02), bank + core::mem::offset_of!(PerTidTelemetryBank, table_03), bank + core::mem::offset_of!(PerTidTelemetryBank, table_04), bank + core::mem::offset_of!(PerTidTelemetryBank, table_05), bank + core::mem::offset_of!(PerTidTelemetryBank, table_06), bank + core::mem::offset_of!(PerTidTelemetryBank, table_07), bank + core::mem::offset_of!(PerTidTelemetryBank, table_08), bank + core::mem::offset_of!(PerTidTelemetryBank, table_09)]; assert_eq!(core::mem::size_of::<SharedU32>(), 4); assert_eq!(core::mem::align_of::<SharedU32>(), 4); assert_eq!(core::mem::size_of::<PerTidTelemetryBank>(), 0x140); assert_eq!(core::mem::align_of::<PerTidTelemetryBank>(), 4); assert_eq!([core::mem::offset_of!(PerTidTelemetryBank, table_00), core::mem::offset_of!(PerTidTelemetryBank, table_01), core::mem::offset_of!(PerTidTelemetryBank, table_02), core::mem::offset_of!(PerTidTelemetryBank, table_03), core::mem::offset_of!(PerTidTelemetryBank, table_04), core::mem::offset_of!(PerTidTelemetryBank, table_05), core::mem::offset_of!(PerTidTelemetryBank, table_06), core::mem::offset_of!(PerTidTelemetryBank, table_07), core::mem::offset_of!(PerTidTelemetryBank, table_08), core::mem::offset_of!(PerTidTelemetryBank, table_09)], [0x000, 0x020, 0x040, 0x060, 0x080, 0x0a0, 0x0c0, 0x0e0, 0x100, 0x120]); assert_eq!(core::mem::size_of::<[SharedU32; 8]>(), 0x20); assert_eq!(core::mem::size_of::<[SharedU32; 8]>() / core::mem::size_of::<SharedU32>(), 8); assert_eq!(core::mem::size_of::<SharedU32>(), 4); assert_eq!(roots, [0x0400_12cc, 0x0400_12ec, 0x0400_130c, 0x0400_132c, 0x0400_134c, 0x0400_136c, 0x0400_138c, 0x0400_13ac, 0x0400_13cc, 0x0400_13ec]); for pair in roots.windows(2) { assert_eq!(pair[0] + core::mem::size_of::<[SharedU32; 8]>(), pair[1]); } assert_eq!(roots[9] + core::mem::size_of::<[SharedU32; 8]>(), 0x0400_140c); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter), 0x0400_12c8); assert_eq!(core::mem::size_of::<RetryPathCounter>(), 4); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter) + core::mem::size_of::<RetryPathCounter>(), 0x0400_12cc); assert_eq!(bank, 0x0400_12cc); assert_eq!(AMPDU_TELEMETRY_COUNTERS.get() + core::mem::size_of::<AmpduTelemetryCounters>(), 0x0400_12c8); assert_eq!(AMPDU_COMPLETION_CONTROL.get(), 0x0400_140c); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); }
    ...
    #[test]
    fn initialized_tx_confirm_aggregation_state_addresses_are_exact() { let image = DTCM_STATE_BASE; let record = image + core::mem::offset_of!(InitializedDtcmPrefix, tx_confirm_aggregation_state); let fields = [record + core::mem::offset_of!(TxConfirmAggregationState, state), record + core::mem::offset_of!(TxConfirmAggregationState, pending_message_raw), record + core::mem::offset_of!(TxConfirmAggregationState, append_cursor_raw)]; assert_eq!(core::mem::size_of::<SharedU32>(), 4); assert_eq!(core::mem::align_of::<SharedU32>(), 4); assert_eq!(core::mem::size_of::<TxConfirmAggregationState>(), 0x0c); assert_eq!(core::mem::align_of::<TxConfirmAggregationState>(), 4); assert_eq!([core::mem::offset_of!(TxConfirmAggregationState, state), core::mem::offset_of!(TxConfirmAggregationState, pending_message_raw), core::mem::offset_of!(TxConfirmAggregationState, append_cursor_raw)], [0x00, 0x04, 0x08]); assert_eq!(fields, [0x0400_1410, 0x0400_1414, 0x0400_1418]); for pair in fields.windows(2) { assert_eq!(pair[0] + core::mem::size_of::<SharedU32>(), pair[1]); } assert_eq!(record, 0x0400_1410); assert_eq!(record + core::mem::size_of::<TxConfirmAggregationState>(), 0x0400_141c); assert_eq!(AMPDU_COMPLETION_CONTROL.get(), 0x0400_140c); assert_eq!(AMPDU_COMPLETION_CONTROL.get() + core::mem::size_of::<AmpduCompletionControl>(), 0x0400_1410); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, configuration_apply_flags), 0x0400_141c); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, control_words), 0x0400_1420); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); }

    #[test]
    fn initialized_configuration_apply_flags_address_is_exact() { let image = DTCM_STATE_BASE; let record = image + core::mem::offset_of!(InitializedDtcmPrefix, configuration_apply_flags); let field = record + core::mem::offset_of!(ConfigurationApplyFlags, flags); assert_eq!(core::mem::size_of::<SharedU32>(), 4); assert_eq!(core::mem::align_of::<SharedU32>(), 4); assert_eq!(core::mem::size_of::<ConfigurationApplyFlags>(), 0x04); assert_eq!(core::mem::align_of::<ConfigurationApplyFlags>(), 4); assert_eq!(core::mem::offset_of!(ConfigurationApplyFlags, flags), 0x00); assert_eq!(record, 0x0400_141c); assert_eq!(field, 0x0400_141c); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, tx_confirm_aggregation_state) + core::mem::size_of::<TxConfirmAggregationState>(), 0x0400_141c); assert_eq!(record + core::mem::size_of::<ConfigurationApplyFlags>(), 0x0400_1420); assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, control_words), 0x0400_1420); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); } #[test] fn initialized_debug_platform_local_tail_addresses_are_exact() { let image = DTCM_STATE_BASE; let opaque = image + core::mem::offset_of!(InitializedDtcmPrefix, pre_phy_channel_threshold_descriptors); let tail = image + core::mem::offset_of!(InitializedDtcmPrefix, debug_platform_local_tail); let fields = [tail + core::mem::offset_of!(DebugPlatformLocalTail, platform_local_word_2c), tail + core::mem::offset_of!(DebugPlatformLocalTail, platform_local_word_30)]; let phy = image + core::mem::offset_of!(InitializedDtcmPrefix, phy_channel_threshold_descriptors); assert_eq!(core::mem::size_of::<SharedU32>(), 0x04); assert_eq!(core::mem::align_of::<SharedU32>(), 4); assert_eq!(core::mem::size_of::<DebugPlatformLocalTail>(), 0x08); assert_eq!(core::mem::align_of::<DebugPlatformLocalTail>(), 4); assert_eq!([core::mem::offset_of!(DebugPlatformLocalTail, platform_local_word_2c), core::mem::offset_of!(DebugPlatformLocalTail, platform_local_word_30)], [0x00, 0x04]); assert_eq!(fields, [0x0400_1454, 0x0400_1458]); assert_eq!(fields[0] + core::mem::size_of::<SharedU32>(), fields[1]); assert_eq!(opaque, 0x0400_1440); assert_eq!(core::mem::size_of::<OpaqueBytes<0x14>>(), 0x14); assert_eq!(opaque + core::mem::size_of::<OpaqueBytes<0x14>>(), 0x0400_1454); assert_eq!(tail + core::mem::size_of::<DebugPlatformLocalTail>(), 0x0400_145c); assert_eq!(phy, 0x0400_145c); assert_eq!(core::mem::size_of::<InitializedDtcmPrefix>(), 0x2078); assert_eq!(core::mem::align_of::<InitializedDtcmPrefix>(), 4); assert_eq!(core::mem::size_of::<DtcmLayout>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<DtcmLayout>(), 4); assert_eq!(core::mem::size_of::<SharedDtcmState>(), DTCM_STATE_SIZE); assert_eq!(core::mem::align_of::<SharedDtcmState>(), 4); }
