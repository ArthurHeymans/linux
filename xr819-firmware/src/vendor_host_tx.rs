...
#[cfg(all(target_arch = "arm", feature = "experimental-list-first-ampdu"))]
#[inline(never)]
pub unsafe fn publish_list_first_depth_two(
    first: &mut RetainedHostTx,
    second: &mut RetainedHostTx,
) -> Result<(u8, u8), AmpduPublishError> {
    if first.phase != HostTxPhase::PasQueued || second.phase != HostTxPhase::PasQueued {
        return Err(AmpduPublishError::Ownership);
    }
    let first_candidate = unsafe { ampdu_candidate(first) };
    if !can_form_ampdu_pair(first_candidate, unsafe { ampdu_candidate(second) }) {
    ...
    if unsafe { read_live_u8(crate::dtcm::mac_retry_hardware_state_mmio_address()) } != 0
        ...
        || !unsafe { program_pipe_eligible(first.context) }
        || !unsafe { program_pipe_eligible(second.context) }
    {
        return Err(AmpduPublishError::Ownership);
    }
    let ac = unsafe { read_host_u8(first.context.access_category()) };
    let pipe = unsafe { read_live_u8(crate::dtcm::access_category_to_queue_unchecked(usize::from(ac)).get() as u32) };
    if pipe >= 4
        || unsafe { read_host_u8(second.context.access_category()) } != ac
        || unsafe { read_live_u8(crate::dtcm::mac_pipe_state_unchecked(usize::from(pipe)).get() as u32) } != 0
        || !crate::tx::host_ampdu_publication_available(pipe)
    {
        return Err(AmpduPublishError::Ownership);
    }
    let head = unsafe { read_live_u32(crate::dtcm::HOST_PAS_RING_HEAD.get() as u32) as u8 & 0x3f };
    let tail = unsafe { read_live_u32(crate::dtcm::HOST_PAS_RING_TAIL.get() as u32) as u8 & 0x3f };
    let Some(first_ring_slot) = (unsafe { queued_pas_ring_slot(first.context, head, tail) }) else {
        return Err(AmpduPublishError::Ownership);
    };
    let Some(second_ring_slot) = (unsafe { queued_pas_ring_slot(second.context, head, tail) }) else {
        return Err(AmpduPublishError::Ownership);
    };
    if first_ring_slot == second_ring_slot {
        return Err(AmpduPublishError::Ownership);
    }
    let pipe_index = usize::from(pipe);
    let slot = unsafe { read_live_u8(crate::dtcm::mac_pipe_current_slot_unchecked(pipe_index).get() as u32) & 3 };
    let slot_record = crate::dtcm::mac_pipe_slot_state_word_unchecked(pipe_index, usize::from(slot)).get() as u32;
    let command = unsafe { read_live_u32(crate::dtcm::mac_pipe_slot_command_unchecked(pipe_index, usize::from(slot)).get() as u32) };
    let hardware_ring = unsafe { read_live_u32(crate::dtcm::mac_pipe_hardware_ring_unchecked(pipe_index).get() as u32) };
    ...
    let contexts = [Some(first.context.raw()), Some(second.context.raw()), None, None];
    let link = usize::from(first_candidate.key.link);
    unsafe {
        write_live_u32(descriptor_head, descriptor_next);
        write_host_u32(first.context.next_in_ampdu(), second.context.pas().raw());
        write_host_u32(second.context.next_in_ampdu(), 0);
        for (ordinal, member) in [first.context, second.context].into_iter().enumerate() {
            write_host_u32(member.control_bits(), (read_host_u32(member.control_bits()) & !0x8000) | 0x20);
            write_host_u32(member.ownership_bits(), read_host_u32(member.ownership_bits()) | 0x80);
            let ring_slot = if ordinal == 0 { first_ring_slot } else { second_ring_slot };
            write_live_u32(crate::dtcm::host_pas_ring_slot_unchecked(usize::from(ring_slot)).get() as u32, 0);
            write_live_u32(
                crate::dtcm::MAC_AGGREGATE_SLOT_TABLES.member_unchecked(link, ordinal).get() as u32,
                member.raw(),
            );
        }
        let mut new_head = head;
        while new_head != tail
            && read_live_u32(crate::dtcm::host_pas_ring_slot_unchecked(usize::from(new_head)).get() as u32) == 0
        {
        ...
        write_live_u8(crate::dtcm::ba_pipe_activity_unchecked(link).get() as u32, 6);
        write_host_u32(first.context.control_bits(), read_host_u32(first.context.control_bits()) | 0x40);
        ...
        if crate::tx::publish_planned_host_ampdu(
            [first.context.raw(), second.context.raw(), 0, 0],
            pipe,
            slot,
        ).is_err() {
            crate::halt_always!();
...
pub unsafe fn publish_list_first_depth_four(
    ...
    let contexts = [
        Some(members[0].context.raw()),
        Some(members[1].context.raw()),
        Some(members[2].context.raw()),
        Some(members[3].context.raw()),
    ];
    let link = usize::from(first_candidate.key.link);
    unsafe {
        write_live_u32(descriptor_head, descriptor_next);
