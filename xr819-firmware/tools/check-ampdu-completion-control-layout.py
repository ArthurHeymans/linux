#!/usr/bin/env python3
"""Drift-evidence gates for initialized A-MPDU completion control.
...
AMPDU_COMPLETION_CONTROL_RANGE = (0x0400140C, 0x04001410)
...
OWNER_FILES = {
    "src/dtcm.rs",
    "tools/check-ampdu-completion-control-layout.py",
}
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {}
FORBIDDEN_FORMS = (
    "AMPDU_COMPLETION_CONTROL_BASE",
    "AMPDU_COMPLETION_ENABLED_BASE",
)

# Regenerated only after reviewing the candidate disassembly and operation order.
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter({0x0400140C: 1})
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RNvNtCsiHlLB2CErfM_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x0400140C): 1,
})
...
def in_family(value: int) -> bool:
    return AMPDU_COMPLETION_CONTROL_RANGE[0] <= value < AMPDU_COMPLETION_CONTROL_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: A-MPDU-completion-control literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized A-MPDU-completion-control form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"A-MPDU COMPLETION CONTROL SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("A-MPDU COMPLETION CONTROL LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "A-MPDU COMPLETION CONTROL LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"A-MPDU-completion-control drift gate failed: {error}")
