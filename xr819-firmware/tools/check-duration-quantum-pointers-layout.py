#!/usr/bin/env python3
"""Drift-evidence gates for initialized duration-quantum register pointers.
...
DURATION_QUANTUM_POINTER_RANGE = (0x040010D4, 0x040010E4)
...
FORBIDDEN_FORMS = (
    "DURATION_QUANTUM_POINTERS_BASE",
    "DURATION_QUANTUM_POINTER_STRIDE",
)

# Regenerated only after reviewing the candidate disassembly and operation order.
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter({0x040010D4: 2})
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RINvNtCs2QKOjPfuf9x_14xr819_firmware2tx32execute_single_probe_publicationNtB2_19VolatileMacPipeMmioEB4_', 0x040010D4): 1,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x040010D4): 1,
})
...
def in_family(value: int) -> bool:
    return DURATION_QUANTUM_POINTER_RANGE[0] <= value < DURATION_QUANTUM_POINTER_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: duration-quantum-pointer literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized duration-quantum-pointer form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"DURATION QUANTUM POINTER SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("DURATION QUANTUM POINTER LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "DURATION QUANTUM POINTER LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"duration-quantum-pointer drift gate failed: {error}")
