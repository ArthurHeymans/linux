#!/usr/bin/env python3
"""Drift-evidence gates for the internal-context prefix overlay.
...
INTERNAL_CONTEXT_PREFIX_RANGE = (0x0400906C, 0x04009080)
...
FORBIDDEN_FORMS = (
    "INTERNAL_CONTEXT_PREFIX_BASE",
    "INTERNAL_CONTEXT_IV_SEED",
)
...
def in_family(value: int) -> bool:
    return INTERNAL_CONTEXT_PREFIX_RANGE[0] <= value < INTERNAL_CONTEXT_PREFIX_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: internal-context-prefix literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized internal-context-prefix form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INTERNAL CONTEXT PREFIX SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INTERNAL CONTEXT PREFIX LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INTERNAL CONTEXT PREFIX LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"internal-context-prefix drift gate failed: {error}")
