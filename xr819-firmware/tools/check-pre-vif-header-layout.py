#!/usr/bin/env python3
"""Drift-evidence gates for the pre-VIF link/aggregation header.
...
PRE_VIF_HEADER_RANGE = (0x04003E78, 0x04003E98)
...
OWNER_FILES = {
    ...
    "src/vif.rs",
    "tools/check-low-mac-pas-layout.py",
    "tools/check-pre-vif-header-layout.py",
    "tools/check-vif-layout.py",
}
...
def in_family(value: int) -> bool:
    return PRE_VIF_HEADER_RANGE[0] <= value < PRE_VIF_HEADER_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: pre-VIF-header literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized pre-VIF-header form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"PRE-VIF HEADER SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("PRE-VIF HEADER LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "PRE-VIF HEADER LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"pre-VIF-header drift gate failed: {error}")
