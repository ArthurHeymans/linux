#!/usr/bin/env python3
"""Source/linked drift evidence for [0x040012c8, 0x040012cc).
...
RANGE = (0x040012C8, 0x040012CC)
OFFSETS = range(0x12C8, 0x12CC)
...
ADJACENT_DECLARATIONS = {
    "tools/check-ampdu-telemetry-layout.py": "(0x040012A0, 0x040012C8)",
    "tools/check-initialized-per-tid-telemetry-bank-layout.py": "(0x040012CC, 0x0400140C)",
}
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter()
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter()
STRUCT = "#[repr(C, align(4))] struct RetryPathCounter { count: SharedU32 }"
REQUIRED = (
    STRUCT,
    "retry_path_counter: RetryPathCounter, per_tid_telemetry_bank: PerTidTelemetryBank",
    "assert_type_layout!(SharedU32, 0x04, 4)",
    "assert_type_layout!(RetryPathCounter, 0x04, 4)",
    "offset_of!(RetryPathCounter, count) == 0x00",
    "offset_of!(InitializedDtcmPrefix, retry_path_counter) == 0x12c8",
    "offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) == 0x12cc",
    ...
    "fn initialized_retry_path_counter_address_is_exact()",
    "size_of::<RetryPathCounter>(), 0x04",
    "align_of::<RetryPathCounter>(), 4",
    "offset_of!(RetryPathCounter, count), 0x00",
    "AMPDU_TELEMETRY_COUNTERS.get() + core::mem::size_of::<AmpduTelemetryCounters>(), 0x0400_12c8",
    "offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank), 0x0400_12cc",
...
PHYSICAL = (0x040012C8, 0x040012CC)
COMPILE_TIME_PHYSICAL_INVENTORY = (
    "assert_type_layout!(RetryPathCounter, 0x04, 4);",
    "assert!(core::mem::offset_of!(RetryPathCounter, count) == 0x00);",
    "assert!(core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter) == 0x12c8);",
    "assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter) + core::mem::offset_of!(RetryPathCounter, count) == 0x0400_12c8);",
    "assert!(DTCM_STATE_BASE + core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter) + core::mem::size_of::<RetryPathCounter>() == 0x0400_12cc);",
    "assert!(core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank) == 0x12cc);",
)
FOCUSED_TEST_INVENTORY = (
    "let image = DTCM_STATE_BASE;",
    "let record = image + core::mem::offset_of!(InitializedDtcmPrefix, retry_path_counter);",
    "let field = record + core::mem::offset_of!(RetryPathCounter, count);",
    "assert_eq!(core::mem::size_of::<SharedU32>(), 0x04);",
    "assert_eq!(core::mem::align_of::<SharedU32>(), 4);",
    "assert_eq!(core::mem::size_of::<RetryPathCounter>(), 0x04);",
    "assert_eq!(core::mem::align_of::<RetryPathCounter>(), 4);",
    "assert_eq!(core::mem::offset_of!(RetryPathCounter, count), 0x00);",
    "assert_eq!(AMPDU_TELEMETRY_COUNTERS.get() + core::mem::size_of::<AmpduTelemetryCounters>(), 0x0400_12c8);",
    "assert_eq!(record, 0x0400_12c8);",
    "assert_eq!(field, 0x0400_12c8);",
    "assert_eq!(record + core::mem::size_of::<RetryPathCounter>(), 0x0400_12cc);",
    "assert_eq!(image + core::mem::offset_of!(InitializedDtcmPrefix, per_tid_telemetry_bank), 0x0400_12cc);",
...
def swapped_once(source: str, left: str, right: str) -> str:
    sentinel = "__RETRY_PATH_COUNTER_LAYOUT_SWAP__"
...
def check_exact_inventory_regression(source: str) -> None:
    ...
    test_scope = named_function(source, "initialized_retry_path_counter_address_is_exact")
    ...
    compile_swap = normalized(compile_scope).replace(
        compile_item, compile_item.replace("0x0400_12c8", "0x0400_12cc"), 1
    )
    test_address_item = normalized(FOCUSED_TEST_INVENTORY[9])
    test_address_swap = normalized(test_scope).replace(
        test_address_item, test_address_item.replace("0x0400_12c8", "0x0400_12cc"), 1
    )
    test_field_item = normalized(FOCUSED_TEST_INVENTORY[2])
    test_field_swap = normalized(test_scope).replace(
        test_field_item, test_field_item.replace("RetryPathCounter, count", "RetryPathCounter, missing_count"), 1
    )
...
def is_owned_literal(value: int) -> bool:
    return in_range(value) or value in OFFSETS
...
def owned_literals(code: str) -> list[int]:
    ...
    return [int(match.group().replace("_", ""), 16) for match in LITERAL.finditer(code)
            if is_owned_literal(int(match.group().replace("_", ""), 16))]


def check_non_owner_literal_regression() -> None:
    # Exercise the same predicate as the repository-wide pass with a non-Rust
    # fixture; otherwise a C/header/assembly relative literal can bypass it.
    fixture = code_only("unsigned x = 0x12c8;", False, False)
    values = [int(match.group().replace("_", ""), 16) for match in LITERAL.finditer(fixture)]
    if not any(is_owned_literal(value) for value in values):
        raise SystemExit("checker self-test failed: non-Rust relative DTCM literal was accepted")
...
def family_aliases(code: str) -> tuple[set[str], list[tuple[str, str, str]]]:
    ...
    views = {"RetryPathCounter", "retry_path_counter",
...
def check_alias_tracking_regression() -> None:
    fixtures = (
        ("const ROOT: usize = retry_path_counter; const NEXT: usize = ROOT; fn leak() -> *mut u32 { NEXT as *mut u32 }", {"ROOT", "NEXT"}, ["leak"]),
        ("static ROOT: usize = 0x0400_12c8; static NEXT: usize = ROOT; fn leak() -> usize { NEXT }", {"ROOT", "NEXT"}, ["leak"]),
        ("const RETRY_OFFSET: usize = 0x12c8;", {"RETRY_OFFSET"}, []),
        ("static RETRY_OFFSET: usize = 0x12cb;", {"RETRY_OFFSET"}, []),
        ("const RETRY_OFFSET: usize = 0x12c8; const NEXT: usize = RETRY_OFFSET; fn leak() -> usize { NEXT }", {"RETRY_OFFSET", "NEXT"}, ["leak"]),
        ("const RETRY_OFFSET: usize = 0x12c8; const ADDRESS: DtcmAddress = DtcmAddress::from_offset(RETRY_OFFSET); fn leak() -> *mut u32 { ADDRESS.get() as *mut u32 }", {"RETRY_OFFSET", "ADDRESS"}, ["leak"]),
        ("type Hidden = RetryPathCounter; type Again = Hidden; fn leak(_: Again) {}", {"Hidden", "Again"}, ["leak"]),
        ("use crate::dtcm::RetryPathCounter as Hidden; const ROOT: usize = retry_path_counter; fn leak(_: Hidden) -> usize { ROOT }", {"Hidden", "ROOT"}, ["leak"]),
        ("use crate::dtcm::{RetryPathCounter as Hidden, retry_path_counter as Root}; const NEXT: usize = Root; fn leak(_: Hidden) -> usize { NEXT }", {"Hidden", "Root", "NEXT"}, ["leak"]),
        ("use crate::{dtcm::{RetryPathCounter as Hidden}}; fn leak(_: Hidden) {}", {"Hidden"}, ["leak"]),
        ("fn leak() -> *mut u32 { (DTCM_STATE_BASE + 0x12c8) as *mut u32 }", set(), ["leak"]),
        ("const ENTRY: usize = DTCM_STATE_BASE + 0x12c8; const NEXT: usize = ENTRY; fn leak() -> *mut u32 { NEXT as *mut u32 }", {"ENTRY", "NEXT"}, ["leak"]),
        ("fn leak() -> *mut u32 { DtcmAddress::from_offset(0x12c8).cast_mut() }", set(), ["leak"]),
        ("const ENTRY: DtcmAddress = DtcmAddress::from_offset(0x12c8); const NEXT: DtcmAddress = ENTRY; fn leak() -> usize { NEXT.get() }", {"ENTRY", "NEXT"}, ["leak"]),
        ("fn leak() -> usize { DtcmAddress::from_offset_unchecked(0x12c8).get() }", set(), ["leak"]),
        ("use crate::{dtcm::{DtcmAddress as HiddenAddress}}; const ENTRY: HiddenAddress = HiddenAddress::from_offset(0x12c8); const NEXT: HiddenAddress = ENTRY; fn leak() -> usize { NEXT.get() }", {"ENTRY", "NEXT"}, ["leak"]),
        ("fn reset() { unsafe { core::ptr::write_volatile(0x0400_12c8 as *mut u32, 0) } }", set(), ["reset"]),
    )
    ...
    rejected_offsets = (
        ("const RETRY_OFFSET: usize = 0x12c8;", {"RETRY_OFFSET"}),
        ("static RETRY_OFFSET: usize = 0x12cb;", {"RETRY_OFFSET"}),
        ("const RETRY_OFFSET: usize = 0x12c8; const NEXT: usize = RETRY_OFFSET;", {"RETRY_OFFSET", "NEXT"}),
        ("const RETRY_OFFSET: usize = 0x12c8; const ADDRESS: DtcmAddress = DtcmAddress::from_offset(RETRY_OFFSET); fn raw() -> *mut u32 { ADDRESS.get() as *mut u32 }", {"RETRY_OFFSET", "ADDRESS"}),
    )
...
def check_source() -> None:
    ...
    compile_scope = mask_test_module(source)
    focused_test = named_function(source, "initialized_retry_path_counter_address_is_exact")
    failures.extend(
    ...
    if not exact_inventory_present(compile_scope, COMPILE_TIME_PHYSICAL_INVENTORY):
        failures.append("src/dtcm.rs: exact contiguous retry-path-counter compile-time assertion inventory changed")
    if focused_test is None:
        failures.append("src/dtcm.rs: focused retry-path counter layout test is missing")
    else:
        failures.extend(
        ...
        if not exact_inventory_present(focused_test, FOCUSED_TEST_INVENTORY):
            failures.append("src/dtcm.rs: exact contiguous focused retry-path-counter test inventory changed")
    ...
    declaration = re.search(r"(?:#\[[^\n]*\]\s*)*#\[repr\(C, align\(4\)\)\]\s*struct\s+RetryPathCounter\s*\{[^}]*\}", source)
    if declaration is None or normalized(declaration.group()) != normalized(STRUCT) or "derive" in declaration.group():
        failures.append("src/dtcm.rs: RetryPathCounter must be the exact private non-derived inventory")
    if source.count("struct RetryPathCounter") != 1 or "pre_per_tid_telemetry_bank:" in source:
        failures.append("src/dtcm.rs: removed opaque word or duplicate retry-path-counter record remains")
    ...
    for kind, name, _ in declarations:
        if name in forbidden_declarations:
            failures.append(f"additional retry-path counter {kind} alias is forbidden: {name}")
    view_pattern = re.compile(rf"\b(?:{'|'.join(sorted(map(re.escape, views)))})\b")
    if re.search(r"\bimpl(?:\s*<[^>]*>)?\s+[^\{]*RetryPathCounter", production):
        failures.append("production impl for RetryPathCounter is forbidden")
    for match in re.finditer(r"\b(?:const|static)\s+(?:mut\s+)?([A-Za-z_][A-Za-z0-9_]*)[^;]*;", production):
        if view_pattern.search(match.group()):
            failures.append(f"direct retry-path counter const/static alias is forbidden: {match.group(1)}")
    for match in re.finditer(r"\b(?:unsafe\s+)?(?:const\s+)?fn\s+([A-Za-z_][A-Za-z0-9_]*(?:retry_path|retry_counter)[A-Za-z0-9_]*)", production, re.I):
        failures.append(f"named retry-path counter production API is forbidden: {match.group(1)}")
    ...
    for relative, code in rust.items():
        for function in functions_consuming_family(code, views):
            failures.append(f"{relative}: production function API over retry-path counter storage is forbidden: {function}")
        for line_number, line in enumerate(code.splitlines(), 1):
            ...
            for value in owned_literals(line):
                if value in OFFSETS and relative != "src/dtcm.rs":
                    failures.append(f"{relative}:{line_number}: raw retry-path-counter DTCM offset 0x{value:x} is forbidden")
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if is_owned_literal(value):
                line = code.count("\n", 0, match.start()) + 1
                kind = "physical" if in_range(value) else "DTCM-relative"
                failures.append(f"{relative}:{line}: retry-path counter {kind} literal {match.group()} is outside reviewed owners")
    if failures: raise SystemExit("\n".join(failures))
    print(f"INITIALIZED RETRY-PATH COUNTER SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if literals != ALLOWED_LINKED_LITERALS or xrefs != ALLOWED_DECODED_XREFS:
        raise SystemExit(f"INITIALIZED RETRY-PATH COUNTER LINKED DRIFT GATE FAILED\nliterals={dict(literals)!r}\nxrefs={dict(xrefs)!r}")
    print("INITIALIZED RETRY-PATH COUNTER LINKED DRIFT-EVIDENCE GATE PASSED literals=0 decoded_xrefs=0")


def main() -> None:
    check_alias_tracking_regression()
    check_non_owner_literal_regression()
...
if __name__ == "__main__":
    try: main()
        ...
        raise SystemExit(f"initialized-retry-path-counter drift gate failed: {error}")
