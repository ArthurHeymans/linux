#!/usr/bin/env python3
"""Drift-evidence gates for PHY gain-source records.
...
PHY_GAIN_SOURCE_RANGE = (0x04002234, 0x040022B8)
...
FORBIDDEN_FORMS = (
    "PHY_GAIN_SOURCE_BASE",
    "PHY_GAIN_SOURCE_STRIDE",
)
...
def in_family(value: int) -> bool:
    return PHY_GAIN_SOURCE_RANGE[0] <= value < PHY_GAIN_SOURCE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: PHY-gain-source literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized PHY-gain-source form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"PHY GAIN SOURCE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("PHY GAIN SOURCE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "PHY GAIN SOURCE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"PHY-gain-source drift gate failed: {error}")
