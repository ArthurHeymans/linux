...
# TXed straight from the driver's status file, used to prove traffic actually
# reached the radio during a phase.
sshpass_txed() {
  grep -oE "^TXed:[[:space:]]+[0-9]+" /sys/kernel/debug/ieee80211/phy0/xr819/status 2>/dev/null \
    | grep -oE "[0-9]+" || echo 0
}
...
# Power save costs a wake cycle per frame. Firmware latency is ~1.35 ms of a
# ~10.8 ms per-frame budget, so most of the time is outside the firmware, and
# the driver status reports "Powermgmt: on" with "Device: asleep".
ip netns exec "$NS" iw dev "$XRIF" set power_save off 2>&1 | tail -1
echo "POWERSAVE=$(ip netns exec "$NS" iw dev "$XRIF" get power_save 2>&1 | tail -1)"
...
sleep 1
echo "=== TCP ==="
ip netns exec "$NS" timeout 45 iperf -c "$SERVER" -t 20 -i 5 2>&1 | tail -8
pkill -f 'iperf -s' 2>/dev/null || true
sleep 1
iperf -s -u -D -B "$SERVER" >/run/iperf-udp-server.log 2>&1 || echo "UDP_SERVER_START_FAILED"
sleep 1
echo "=== UDP 8Mbit offered ==="
# A UDP client with no server reachable still reports the OFFERED rate, because
# UDP has no handshake and nothing pushes back. That produced a fake "8.39
# Mbit/s" (8M + headers) in every dead-link run. Two independent checks below:
# a server report line (jitter + lost/total, only printed when datagrams
# actually arrived) and firmware TXed movement across the phase.
txed_before=$(sshpass_txed)
ip netns exec "$NS" timeout 45 iperf -c "$SERVER" -u -b 8M -t 20 -i 5 2>&1 | tail -12
txed_after=$(sshpass_txed)
echo "UDP_VALIDATION txed_before=$txed_before txed_after=$txed_after"
echo "IPERF_RC=$?"
pkill -f 'iperf -s' 2>/dev/null || true
# Post-traffic only. A mid-traffic debugfs read destroys this firmware (1.22
# Mbit/s -> no link, while vendor is untouched), so nothing here runs while
# frames are moving. Signal and bitrate say whether the air changed; survey
# noise and channel-busy time say whether the band got congested, which is the
# difference between "our firmware regressed" and "the environment moved".
echo "POST_STATION:"
ip netns exec "$NS" iw dev "$XRIF" station dump 2>&1 | grep -iE "signal|bitrate|failed|retries|tx bytes|tx packets" | sed 's/^/  /'
echo "POST_SURVEY:"
ip netns exec "$NS" iw dev "$XRIF" survey dump 2>&1 | grep -iE "frequency|noise|busy|active|receive|transmit" | head -8 | sed 's/^/  /' || echo "  (survey unavailable: link down)"
sample after-iperf
