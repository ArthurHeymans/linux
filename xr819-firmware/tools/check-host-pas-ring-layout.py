#!/usr/bin/env python3
"""Drift-evidence gates for the retained host PAS ring.
...
HOST_PAS_RING_RANGE = (0x04001578, 0x04001680)
...
OWNER_FILES = {
    ...
    "tools/check-initialized-pre-host-pas-radio-stop-word-layout.py",
}
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {}
FORBIDDEN_FORMS = (
    "HOST_PAS_RING_BASE",
    "HOST_PAS_RING_SLOT_BASE",
    "HOST_PAS_RING_SLOT_STRIDE",
)
...
# reservation helper. Concurrent-pipe candidate selection moves the two
# caller-side ring-root loads from rust_main into the typed
...
def in_family(value: int) -> bool:
    return HOST_PAS_RING_RANGE[0] <= value < HOST_PAS_RING_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: host-PAS-ring literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized host-PAS-ring form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"HOST PAS RING SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("HOST PAS RING LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "HOST PAS RING LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"host-PAS-ring drift gate failed: {error}")
