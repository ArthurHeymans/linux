#!/usr/bin/env python3
"""Drift-evidence gates for initialized HIF/coalescing control state.
...
INITIALIZED_HIF_CONTROL_RANGE = (0x040011AC, 0x040011BC)
...
FORBIDDEN_FORMS = (
    "INITIALIZED_HIF_CONTROL_BASE",
    "HIF_COALESCE_CONFIG_BASE",
)
...
def in_family(value: int) -> bool:
    return INITIALIZED_HIF_CONTROL_RANGE[0] <= value < INITIALIZED_HIF_CONTROL_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: initialized-HIF-control literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized initialized-HIF-control form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INITIALIZED HIF CONTROL SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INITIALIZED HIF CONTROL LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INITIALIZED HIF CONTROL LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"initialized-HIF-control drift gate failed: {error}")
