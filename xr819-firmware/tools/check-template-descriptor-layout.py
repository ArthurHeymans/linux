#!/usr/bin/env python3
"""Drift-evidence gates for retained template-frame descriptors.
...
TEMPLATE_DESCRIPTOR_RANGE = (0x04003050, 0x040030D0)
...
FORBIDDEN_FORMS = (
    "TEMPLATE_DESCRIPTOR_BASE",
    "TEMPLATE_DESCRIPTOR_STRIDE",
)
...
def in_family(value: int) -> bool:
    return TEMPLATE_DESCRIPTOR_RANGE[0] <= value < TEMPLATE_DESCRIPTOR_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: template-descriptor literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized template-descriptor form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"TEMPLATE DESCRIPTOR SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("TEMPLATE DESCRIPTOR LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "TEMPLATE DESCRIPTOR LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"template-descriptor drift gate failed: {error}")
