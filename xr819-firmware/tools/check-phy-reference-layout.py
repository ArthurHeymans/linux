#!/usr/bin/env python3
"""Drift-evidence gates for PHY calibration reference words.
...
PHY_REFERENCE_RANGE = (0x0400993C, 0x0400994C)
...
FORBIDDEN_FORMS = (
    "PHY_REFERENCE_BASE",
    "PHY_REFERENCE_STRIDE",
)

# Regenerated only after reviewing the candidate disassembly and operation order.
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter(
    {0x0400993C: 1, 0x04009945: 1}
)
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter(
    {
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware14vendor_host_tx22release_pending_to_pas', 0x04009945): 2,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy23run_iq_calibration_core', 0x0400993C): 1,
    }
)
...
def in_family(value: int) -> bool:
    return PHY_REFERENCE_RANGE[0] <= value < PHY_REFERENCE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: PHY-reference literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized PHY-reference form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"PHY REFERENCE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("PHY REFERENCE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "PHY REFERENCE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"PHY-reference drift gate failed: {error}")
