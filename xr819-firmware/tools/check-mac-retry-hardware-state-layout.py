#!/usr/bin/env python3
"""Drift-evidence gates for retained MAC retry hardware state.
...
MAC_RETRY_HARDWARE_STATE_RANGE = (0x04001E6C, 0x04001E70)
...
OWNER_FILES = {
    "src/dtcm.rs",
    "tools/check-mac-retry-hardware-state-layout.py",
    "tools/check-initialized-mac-pipe-tails-layout.py",
}
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {}
FORBIDDEN_FORMS = (
    "MAC_RETRY_HARDWARE_STATE_BASE",
    "MAC_RETRY_HARDWARE_CONTROL_BASE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter(
    {
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware14vendor_host_tx34reserve_non_aggregate_scheduler_at', 0x04001E6C): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware2tx28service_mac_event_drain_tail', 0x04001E6C): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x04001E6C): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3mac31initialize_vendor_startup_state', 0x04001E6C): 1,
...
def in_family(value: int) -> bool:
    return MAC_RETRY_HARDWARE_STATE_RANGE[0] <= value < MAC_RETRY_HARDWARE_STATE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: MAC-retry-hardware-state literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized MAC-retry-hardware-state form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"MAC RETRY HARDWARE STATE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("MAC RETRY HARDWARE STATE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "MAC RETRY HARDWARE STATE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"MAC-retry-hardware-state drift gate failed: {error}")
