#!/usr/bin/env python3
"""Drift-evidence gates for wake-context and duration-source state.
...
WAKE_CONTEXT_RANGE = (0x040035E0, 0x04003674)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter(
    {('_RNvNtCsiHlLB2CErfM_14xr819_firmware3mac23reinitialize_after_wake', 0x04003670): 2}
)
...
def in_family(value: int) -> bool:
    return WAKE_CONTEXT_RANGE[0] <= value < WAKE_CONTEXT_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: wake-context literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized wake-context form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"WAKE CONTEXT SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("WAKE CONTEXT LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "WAKE CONTEXT LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"wake-context drift gate failed: {error}")
