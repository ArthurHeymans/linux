#!/usr/bin/env python3
"""Drift-evidence gates for initialized MAC beacon state.
...
MAC_BEACON_STATE_RANGE = (0x04001A80, 0x04001AC0)
...
FORBIDDEN_FORMS = (
    "MAC_BEACON_STATE_BASE",
    "MAC_BEACON_RESPONSE_COMMANDS_BASE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RNvNtCsiHlLB2CErfM_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x04001AA8): 9,
...
def in_family(value: int) -> bool:
    return MAC_BEACON_STATE_RANGE[0] <= value < MAC_BEACON_STATE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: MAC-beacon-state literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized MAC-beacon-state form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"MAC BEACON STATE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("MAC BEACON STATE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "MAC BEACON STATE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"MAC-beacon-state drift gate failed: {error}")
