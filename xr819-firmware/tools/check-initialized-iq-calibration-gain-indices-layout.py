#!/usr/bin/env python3
"""Drift-evidence gates for initialized IQ-calibration gain-index table.
...
INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_RANGE = (0x04000E18, 0x04000E48)
SYNTHESIZED = {0x0E18, 0x0E48}
...
FORBIDDEN_FORMS = (
    "INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_BASE",
    "INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_ROOT",
    "INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_TABLE_BASE",
    "INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_TABLE_ROOT",
    "INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_COUNT",
    "INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_STRIDE",
    "IQ_CALIBRATION_GAIN_INDICES_DTCM_BASE",
    "IQ_CALIBRATION_GAIN_INDICES_DTCM_ROOT",
    "IQ_CALIBRATION_GAIN_INDICES_DTCM_COUNT",
    "IQ_CALIBRATION_GAIN_INDICES_DTCM_STRIDE",
)
...
def in_family(value: int) -> bool:
    return INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_RANGE[0] <= value < INITIALIZED_IQ_CALIBRATION_GAIN_INDICES_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: initialized-IQ-calibration-gain-indices literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized initialized-IQ-calibration-gain-indices form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INITIALIZED IQ CALIBRATION GAIN INDICES SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INITIALIZED IQ CALIBRATION GAIN INDICES LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INITIALIZED IQ CALIBRATION GAIN INDICES LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"initialized-IQ-calibration-gain-indices drift gate failed: {error}")
