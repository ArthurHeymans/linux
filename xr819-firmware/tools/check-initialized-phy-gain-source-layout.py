#!/usr/bin/env python3
"""Drift-evidence gates for initialized PHY gain-source records.
...
FORBIDDEN_FORMS = (
    "INITIALIZED_PHY_GAIN_SOURCE_BASE",
    "INITIALIZED_PHY_GAIN_SOURCE_ROOT",
    "INITIALIZED_PHY_GAIN_SOURCE_TABLE_BASE",
    "INITIALIZED_PHY_GAIN_SOURCE_TABLE_ROOT",
    "INITIALIZED_PHY_GAIN_SOURCE_RECORDS_BASE",
    "INITIALIZED_PHY_GAIN_SOURCE_RECORDS_ROOT",
    "INITIALIZED_PHY_GAIN_SOURCE_PHYSICAL_BASE",
    "INITIALIZED_PHY_GAIN_SOURCE_VIEW_BASE",
    "INITIALIZED_PHY_GAIN_SOURCE_VIEW_STRIDE",
    "INITIALIZED_PHY_GAIN_SOURCE_RECORD_STRIDE",
    "INITIALIZED_PHY_GAIN_SOURCE_PHYSICAL_RECORD_STRIDE",
)
...
def in_family(value: int) -> bool:
    return INITIALIZED_PHY_GAIN_SOURCE_RANGE[0] <= value < INITIALIZED_PHY_GAIN_SOURCE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: initialized-PHY-gain-source literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized initialized-PHY-gain-source form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INITIALIZED PHY GAIN SOURCE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INITIALIZED PHY GAIN SOURCE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INITIALIZED PHY GAIN SOURCE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"initialized-PHY-gain-source drift gate failed: {error}")
