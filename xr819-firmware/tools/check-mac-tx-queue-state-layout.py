#!/usr/bin/env python3
"""Drift-evidence gates for retained MAC TX queue state.
...
MAC_TX_QUEUE_STATE_RANGE = (0x040018D0, 0x040018D8)
...
OWNER_FILES = {
    ...
    "tools/check-mac-tx-queue-state-layout.py",
}
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {}
FORBIDDEN_FORMS = (
    "MAC_TX_QUEUE_STATE_BASE",
    "MAC_TX_QUEUE_HEAD_BASE",
    "MAC_TX_QUEUE_TAIL_BASE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3mac31initialize_vendor_startup_state', 0x040018D0): 1,
})
...
def in_family(value: int) -> bool:
    return MAC_TX_QUEUE_STATE_RANGE[0] <= value < MAC_TX_QUEUE_STATE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: MAC-TX-queue-state literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized MAC-TX-queue-state form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"MAC TX QUEUE STATE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("MAC TX QUEUE STATE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "MAC TX QUEUE STATE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"MAC-TX-queue-state drift gate failed: {error}")
