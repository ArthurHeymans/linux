#!/usr/bin/env python3
"""Drift-evidence gates for initialized TX duration and rate tables.
...
INITIALIZED_TX_RATE_TABLE_RANGE = (0x04000138, 0x040001C0)
...
FORBIDDEN_FORMS = (
    "TX_DURATION_TIMING_TABLE_BASE",
    "RATE_ENCODING_TABLE_BASE",
    "RATE_ATTRIBUTE_TABLE_BASE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RINvNtCsiHlLB2CErfM_14xr819_firmware2tx27build_single_frame_durationNtB2_19VolatileMacPipeMmioEB4_', 0x04000138): 1,
    ('_RNvNtCsiHlLB2CErfM_14xr819_firmware2tx30emit_prepared_probe_descriptor', 0x04000194): 1,
    ('_RNvNtCsiHlLB2CErfM_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x04000194): 2,
    ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3mac24initialize_tx_pipe_state', 0x04000138): 1,
})
...
def in_family(value: int) -> bool:
    return INITIALIZED_TX_RATE_TABLE_RANGE[0] <= value < INITIALIZED_TX_RATE_TABLE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: initialized-TX-rate-table literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized initialized-TX-rate-table form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INITIALIZED TX RATE TABLE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INITIALIZED TX RATE TABLE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INITIALIZED TX RATE TABLE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"initialized-TX-rate-table drift gate failed: {error}")
