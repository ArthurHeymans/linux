#!/usr/bin/env python3
"""Drift-evidence gates for PHY table pointers and calibration controls.
...
PHY_TABLE_CONTROL_RANGE = (0x040099DC, 0x04009A0C)
...
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {
    "src/bin/hif_extension_probe.rs": {0x040099F4},
}
FORBIDDEN_FORMS = (
    "PHY_TABLE_CONTROL_BASE",
    "PHY_TABLE_CONTROL_STRIDE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter(
    {
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy24begin_channel_transition', 0x04009A08): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy29initialize_mac_software_state', 0x04009A08): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy35run_vendor_dynamic_mode_calibration', 0x04009A08): 2,
    }
)
...
def in_family(value: int) -> bool:
    return PHY_TABLE_CONTROL_RANGE[0] <= value < PHY_TABLE_CONTROL_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: PHY-table-control literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized PHY-table-control form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"PHY TABLE CONTROL SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("PHY TABLE CONTROL LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "PHY TABLE CONTROL LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"PHY-table-control drift gate failed: {error}")
