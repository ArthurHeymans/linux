#!/usr/bin/env python3
"""Drift-evidence gates for initialized completion words.
...
INITIALIZED_COMPLETION_WORD_RANGE = (0x04000260, 0x04000288)
...
FORBIDDEN_FORMS = (
    "VISIBLE_COMPLETION_WORDS_BASE",
    "VISIBLE_COMPLETION_WORD_STRIDE",
)
...
def in_family(value: int) -> bool:
    return INITIALIZED_COMPLETION_WORD_RANGE[0] <= value < INITIALIZED_COMPLETION_WORD_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: initialized-completion-word literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized initialized-completion-word form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INITIALIZED COMPLETION WORD SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INITIALIZED COMPLETION WORD LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INITIALIZED COMPLETION WORD LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"initialized-completion-word drift gate failed: {error}")
