#!/usr/bin/env python3
"""Drift-evidence gates for initialized IRQ callback words.
...
INITIALIZED_IRQ_CALLBACK_RANGE = (0x040011BC, 0x0400123C)
...
OWNER_FILES = {
    "src/dtcm.rs",
    "tools/check-initialized-hif-control-layout.py",
    "tools/check-initialized-irq-callbacks-layout.py",
}
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {}
FORBIDDEN_FORMS = (
    "IRQ_CALLBACK_TABLE_BASE",
    "IRQ_CALLBACK_TABLE_STRIDE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware8platform35register_post_activation_interrupts', 0x040011BC): 1,
...
def in_family(value: int) -> bool:
    return INITIALIZED_IRQ_CALLBACK_RANGE[0] <= value < INITIALIZED_IRQ_CALLBACK_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: initialized-IRQ-callback literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized initialized-IRQ-callback form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INITIALIZED IRQ CALLBACK SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INITIALIZED IRQ CALLBACK LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INITIALIZED IRQ CALLBACK LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"initialized-IRQ-callback drift gate failed: {error}")
