#!/usr/bin/env python3
"""Drift-evidence gates for PHY threshold descriptors and gain records.
...
FORBIDDEN_FORMS = (
    "PHY_CHANNEL_THRESHOLD_DESCRIPTORS_BASE",
    "PHY_GAIN_PROGRAMMING_RECORDS_BASE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy25program_all_tx_gain_slots', 0x0400147A): 1,
})
...
def in_family(value: int) -> bool:
    return PHY_DESCRIPTOR_GAIN_RECORD_RANGE[0] <= value < PHY_DESCRIPTOR_GAIN_RECORD_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: PHY-descriptor-gain-record literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized PHY-descriptor-gain-record form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"PHY DESCRIPTOR GAIN RECORD SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("PHY DESCRIPTOR GAIN RECORD LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "PHY DESCRIPTOR GAIN RECORD LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"PHY-descriptor-gain-record drift gate failed: {error}")
