...
if [[ -n "${XR819_INITIALIZED_RETRY_PATH_COUNTER_PARENT_ELF:-}" ]]; then
  echo "== complete reviewed initialized-retry-path-counter parent text-symbol gate =="
  python3 tools/check-hot-codegen.py \
    --manifest tools/initialized-retry-path-counter-codegen-manifest.json \
    "$XR819_INITIALIZED_RETRY_PATH_COUNTER_PARENT_ELF" "$ELF"
else
  echo "== initialized-retry-path-counter codegen gate skipped: set XR819_INITIALIZED_RETRY_PATH_COUNTER_PARENT_ELF to the exact parent ELF =="
fi
...
if [[ -n "${XR819_VENDOR_DEBUG_TABLES_PARENT_ELF:-}" ]]; then
  echo "== supplemental vendor-debug-tables exact-parent text-symbol gate =="
  python3 tools/check-hot-codegen.py \
    --manifest tools/vendor-debug-tables-codegen-manifest.json \
    "$XR819_VENDOR_DEBUG_TABLES_PARENT_ELF" "$ELF"
else
  echo "== vendor-debug-tables codegen gate skipped: set XR819_VENDOR_DEBUG_TABLES_PARENT_ELF to the exact parent ELF; this supplemental symbol gate does not replace complete-file identity =="
fi

if [[ -n "${XR819_COMPLETION_RING_CURSOR_PARENT_ELF:-}" ]]; then
  echo "== supplemental completion/ring-cursor exact-parent text-symbol gate =="
  python3 tools/check-hot-codegen.py \
    --manifest tools/completion-ring-cursor-codegen-manifest.json \
    "$XR819_COMPLETION_RING_CURSOR_PARENT_ELF" "$ELF"
else
  echo "== completion/ring-cursor codegen gate skipped: set XR819_COMPLETION_RING_CURSOR_PARENT_ELF to the exact parent ELF; this supplemental symbol gate does not replace complete-file identity =="
fi

if [[ -n "${XR819_PAS_RATE_STATIC_TABLES_PARENT_ELF:-}" ]]; then
  echo "== supplemental pas-rate-static-tables exact-parent text-symbol gate =="
  python3 tools/check-hot-codegen.py \
    --manifest tools/pas-rate-static-tables-codegen-manifest.json \
    "$XR819_PAS_RATE_STATIC_TABLES_PARENT_ELF" "$ELF"
else
  echo "== pas-rate-static-tables codegen gate skipped: set XR819_PAS_RATE_STATIC_TABLES_PARENT_ELF to the exact parent ELF; this supplemental symbol gate does not replace complete-file identity =="
fi
...
if [[ -n "${XR819_INITIALIZED_DEBUG_PLATFORM_LOCAL_TAIL_PARENT_ELF:-}" ]]; then
  echo "== complete reviewed initialized-debug-platform-local-tail parent text-symbol gate =="
  python3 tools/check-hot-codegen.py \
