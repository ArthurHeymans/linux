#!/usr/bin/env python3
"""Drift-evidence gates for runtime register-context and PAS backoff state.
...
RUNTIME_REGISTER_BACKOFF_RANGE = (0x04002078, 0x04002094)
...
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {
    "src/platform.rs": {0x04002078},
}
FORBIDDEN_FORMS = (
    "RUNTIME_REGISTER_CONTEXT_BASE",
    "PAS_BACKOFF_OVERRIDE_BASE",
)

# Regenerated only after reviewing the candidate disassembly and operation order.
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter(
    {0x04002078: 1, 0x04002088: 1}
)
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter(
    {
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3mac23reprogram_after_channel', 0x04002078): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3vif17reset_pas_backoff', 0x04002088): 1,
    }
)
...
def in_family(value: int) -> bool:
    return RUNTIME_REGISTER_BACKOFF_RANGE[0] <= value < RUNTIME_REGISTER_BACKOFF_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: runtime-register/backoff literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized runtime-register/backoff form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"RUNTIME REGISTER BACKOFF SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("RUNTIME REGISTER BACKOFF LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "RUNTIME REGISTER BACKOFF LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"runtime-register/backoff drift gate failed: {error}")
