#!/usr/bin/env python3
"""Drift-evidence gates for the initialized TKIP S-box tables.
...
complete consumer closure: register-computed retained references are not recovered,
and retained vendor/IRQ ownership remains outside this checker.
"""
...
TKIP_SBOX_RANGE = (0x04000310, 0x04000710)
...
OWNER_FILES = {
    "src/dtcm.rs",
    "tools/check-tkip-sbox-layout.py",
}
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {}
FORBIDDEN_FORMS = (
    "TKIP_SBOX_LOW_BASE",
    "TKIP_SBOX_HIGH_BASE",
    "TKIP_SBOX_TABLE_STRIDE",
)
...
def in_family(value: int) -> bool:
    return TKIP_SBOX_RANGE[0] <= value < TKIP_SBOX_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: TKIP-S-box literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized TKIP-S-box form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"TKIP S-BOX SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("TKIP S-BOX LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "TKIP S-BOX LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"TKIP-S-box drift gate failed: {error}")
