#!/usr/bin/env python3
"""Drift-evidence gates for PHY channel-cache and calibration state.
...
PHY_CHANNEL_CACHE_RANGE = (0x040099AC, 0x040099DC)
...
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {
    "src/bin/hif_extension_probe.rs": {0x040099D4},
}
FORBIDDEN_FORMS = (
    "PHY_CHANNEL_CACHE_BASE",
    "PHY_CHANNEL_CACHE_STRIDE",
)

# Regenerated only after reviewing the candidate disassembly and operation order.
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter(
    {0x040099C4: 2, 0x040099CE: 2, 0x040099D0: 2, 0x040099D8: 2}
)
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter(
    {
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy24begin_channel_transition', 0x040099D0): 6,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy29initialize_mac_software_state', 0x040099CE): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy29initialize_mac_software_state', 0x040099D8): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy32channel_tx_power_from_rate_table', 0x040099D8): 2,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3phy35run_vendor_dynamic_mode_calibration', 0x040099C4): 2,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware4scan7service', 0x040099CE): 1,
...
def in_family(value: int) -> bool:
    return PHY_CHANNEL_CACHE_RANGE[0] <= value < PHY_CHANNEL_CACHE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: PHY-channel-cache literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized PHY-channel-cache form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"PHY CHANNEL CACHE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("PHY CHANNEL CACHE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "PHY CHANNEL CACHE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"PHY-channel-cache drift gate failed: {error}")
