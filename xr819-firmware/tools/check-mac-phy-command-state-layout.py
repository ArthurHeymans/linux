#!/usr/bin/env python3
"""Drift-evidence gates for initialized MAC PHY command state.
...
MAC_PHY_COMMAND_STATE_RANGE = (0x04001D10, 0x04001D5C)
...
OWNER_FILES = {
    "src/dtcm.rs",
    "tools/check-mac-phy-command-state-layout.py",
    "tools/check-initialized-mac-aggregate-slot-tables-layout.py",
}
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {}
FORBIDDEN_FORMS = (
    "MAC_PHY_COMMAND_STATE_BASE",
    "MAC_PHY_OPERATION_ROOT_BASE",
    "MAC_PHY_DISPATCH_COMMAND_BASE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ...
    ('_RNvNtCsbx17WDetRei_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x04001D40): 4,
...
def in_family(value: int) -> bool:
    return MAC_PHY_COMMAND_STATE_RANGE[0] <= value < MAC_PHY_COMMAND_STATE_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: MAC-PHY-command-state literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized MAC-PHY-command-state form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"MAC PHY COMMAND STATE SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("MAC PHY COMMAND STATE LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "MAC PHY COMMAND STATE LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"MAC-PHY-command-state drift gate failed: {error}")
