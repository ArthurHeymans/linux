#!/usr/bin/env python3
"""Drift-evidence gates for the five embedded timers in each VIF record.
...
VIF_STRIDE = 0x3B0
VIF_COUNT = 3
TIMER_OFFSETS = (0x0B0, 0x0C4, 0x0D8, 0x184, 0x198)
TIMER_SIZE = 0x14
...
OWNER_FILES = {
    ...
    "tools/check-vif-timer-layout.py",
}
...
def in_family(value: int) -> bool:
    return any(
        VIF_BASE + record * VIF_STRIDE + offset <= value
        < VIF_BASE + record * VIF_STRIDE + offset + TIMER_SIZE
        for record in range(VIF_COUNT)
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: VIF-timer literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized VIF-timer form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"VIF TIMER SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("VIF TIMER LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "VIF TIMER LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"VIF-timer drift gate failed: {error}")
