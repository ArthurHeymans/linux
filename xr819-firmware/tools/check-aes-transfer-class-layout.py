#!/usr/bin/env python3
"""Drift-evidence gates for the initialized AES transfer-class table.
...
complete consumer closure: computed and indirect references are not recovered,
and retained vendor/IRQ consumers and ownership remain outside this checker.
"""
...
AES_TRANSFER_CLASS_RANGE = (0x04000804, 0x04000830)
...
FORBIDDEN_FORMS = (
    "AES_TRANSFER_CLASS_BASE",
    "AES_TRANSFER_CLASS_TABLE_BASE",
    "AES_TRANSFER_CLASS_STRIDE",
)
...
def in_family(value: int) -> bool:
    return AES_TRANSFER_CLASS_RANGE[0] <= value < AES_TRANSFER_CLASS_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: AES-transfer-class literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized AES-transfer-class form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"AES TRANSFER-CLASS SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("AES TRANSFER-CLASS LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "AES TRANSFER-CLASS LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"AES-transfer-class drift gate failed: {error}")
