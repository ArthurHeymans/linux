#!/usr/bin/env python3
"""Drift-evidence gates for initialized queue/pipe mapping tables.
...
QUEUE_PIPE_MAPPING_RANGE = (0x040002D8, 0x040002E4)
...
FORBIDDEN_FORMS = (
    "QUEUE_PIPE_MAPPINGS_BASE",
    "QUEUE_TO_ACCESS_CATEGORY_BASE",
    "ACCESS_CATEGORY_TO_QUEUE_BASE",
)

# Regenerated only after reviewing the candidate disassembly and operation order.
# The host TX path derives the preceding completion-presence symbol through a
# 0x040002e0 anchor with negative offsets; its stores remain in 0x0260..0x0288.
# Concurrent-pipe candidate selection moves the reviewed queue-map read from
# rust_main into the typed `scheduler_live_diagnostic` helper.
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter({0x040002D8: 1, 0x040002DC: 6, 0x040002E0: 3})
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter(
    {
        ('_RINvNtCs2QKOjPfuf9x_14xr819_firmware2tx32execute_single_probe_publicationNtB2_19VolatileMacPipeMmioEB4_', 0x040002DC): 1,
        ('_RNvMs_NtCs2QKOjPfuf9x_14xr819_firmware14host_tx_driverNtB4_12HostTxDriver5admit', 0x040002DC): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware14vendor_host_tx34reserve_non_aggregate_scheduler_at', 0x040002E0): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware2tx21prepare_probe_context', 0x040002DC): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware2tx26service_host_management_tx', 0x040002DC): 2,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware2tx27prepare_context_publication', 0x040002E0): 1,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x040002DC): 2,
        ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3mac24initialize_tx_pipe_state', 0x040002D8): 2,
...
def in_family(value: int) -> bool:
    return QUEUE_PIPE_MAPPING_RANGE[0] <= value < QUEUE_PIPE_MAPPING_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: queue/pipe-mapping literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized queue/pipe-mapping form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"QUEUE/PIPE MAPPING SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("QUEUE/PIPE MAPPING LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "QUEUE/PIPE MAPPING LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"queue/pipe-mapping drift gate failed: {error}")
