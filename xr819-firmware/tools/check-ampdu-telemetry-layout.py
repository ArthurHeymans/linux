#!/usr/bin/env python3
"""Drift-evidence gates for initialized A-MPDU telemetry state.
...
AMPDU_TELEMETRY_RANGE = (0x040012A0, 0x040012C8)
...
FORBIDDEN_FORMS = (
    "AMPDU_TELEMETRY_BASE",
    "AMPDU_COUNTERS_BASE",
)

# Regenerated only after reviewing the candidate disassembly and operation order.
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter({0x040012A0: 1})
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RNvNtCsiHlLB2CErfM_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x040012A0): 2,
})
...
def in_family(value: int) -> bool:
    return AMPDU_TELEMETRY_RANGE[0] <= value < AMPDU_TELEMETRY_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: A-MPDU-telemetry literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized A-MPDU-telemetry form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"A-MPDU TELEMETRY SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("A-MPDU TELEMETRY LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "A-MPDU TELEMETRY LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"A-MPDU-telemetry drift gate failed: {error}")
