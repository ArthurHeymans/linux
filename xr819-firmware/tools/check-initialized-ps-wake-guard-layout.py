#!/usr/bin/env python3
"""Source/linked drift evidence for exactly [0x040010E4, 0x040010F8).

Power-save wake-guard words rooted at literal-pool word DAT_00004e9c
(0x040010e4), with three evidenced accesses:

- word_10e4: read by vendor ps_schedule_next_tbtt_wake as the *root + 10
  minimum guard applied to the computed TBTT wake deadline.
- word_10ec: read twice inside vendor task_143a6 (the scheduler event task):
  once summed with word_10f0 plus 0x400 and stored into the structure at
  0x04008afc, once doubled and subtracted from a wake deadline bound.
- word_10f0: only in the sum above.

The words at +0x04 and +0x10 have no observed accessor and stay opaque.
task_143a6 dispatches through a jump table Ghidra could not recover; the two
branches above were located by scanning every PC-relative load of both pool
words holding this root (DAT_000145f4, DAT_00014890) across the whole
binary -- a complete accessor enumeration by construction.
...
RANGE = (0x040010E4, 0x040010F8)
OFFSETS = range(0x10E4, 0x10F8)
...
OWNER_FILES = {
    "src/dtcm.rs",
    "tools/check-initialized-ps-wake-guard-layout.py",
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ("_RNvNtCsiHlLB2CErfM_14xr819_firmware3mac24initialize_tx_pipe_state", 0x040010E4): 1,
})
...
REQUIRED = (
    ...
    "offset_of!(PsWakeGuardWords, word_10e4) == 0",
    ...
    "size_of::<PsWakeGuardWords>(), 0x14",
    "align_of::<PsWakeGuardWords>(), 4",
    "core::mem::offset_of!(PsWakeGuardWords, word_10ec)",
    "core::mem::offset_of!(PsWakeGuardWords, word_10f0)",
    "record + core::mem::offset_of!(PsWakeGuardWords, word_10ec), 0x0400_10ec",
    "record + core::mem::offset_of!(PsWakeGuardWords, word_10f0), 0x0400_10f0",
    "opaque_10)], [0x00, 0x08, 0x0c, 0x10]",
    "record, 0x0400_10e4",
...
COMPILE_TIME_PHYSICAL_INVENTORY = (
    ...
    "assert!(core::mem::offset_of!(PsWakeGuardWords, word_10ec) == 0x08);",
    "assert!(core::mem::offset_of!(PsWakeGuardWords, word_10f0) == 0x0c);",
    "assert!(core::mem::offset_of!(PsWakeGuardWords, opaque_10) == 0x10);",
)
FOCUSED_TEST_INVENTORY = (
    ...
    'assert_eq!(record, 0x0400_10e4);',
    'assert_eq!(record + core::mem::offset_of!(PsWakeGuardWords, word_10ec), 0x0400_10ec);',
    'assert_eq!(record + core::mem::offset_of!(PsWakeGuardWords, word_10f0), 0x0400_10f0);',
...
def check_exact_inventory_regression(source: str) -> None:
    ...
    test_scope = named_function(source, "ps_wake_guard_words_are_exact")
    ...
    compile_swap = normalized(compile_scope).replace(
        compile_item, compile_item.replace("== 0x10", "== 0x0c"), 1
    )
    test_address_item = normalized(FOCUSED_TEST_INVENTORY[5])
    test_address_swap = normalized(test_scope).replace(
        test_address_item,
        swapped_once(test_address_item, "0x0400_10e4", "0x0400_10e2"),
        1,
    )
    test_extent_item = normalized(FOCUSED_TEST_INVENTORY[8])
    test_extent_swap = normalized(test_scope).replace(
        test_extent_item,
        swapped_once(test_extent_item, "0x0400_10f8", "0x0400_10f6"),
        1,
    )
...
def check_alias_tracking_regression() -> None:
    fixtures = (
        ("type A = PsWakeGuardWords; type B = A; fn leak(_: B) {}", {"A", "B"}, ["leak"]),
        ("static ROOT: usize = 0x0400_10e4; static NEXT: usize = ROOT; fn leak() -> usize { NEXT }", {"ROOT", "NEXT"}, ["leak"]),
        ("type Hidden = PsWakeGuardWords; type Again = Hidden; fn leak(_: Again) {}", {"Hidden", "Again"}, ["leak"]),
        ("use crate::dtcm::PsWakeGuardWords as Hidden; type Again = Hidden; fn leak(_: Again) {}", {"Hidden", "Again"}, ["leak"]),
        ("use crate::dtcm::{PsWakeGuardWords as H1}; type H2 = H1; const N: usize = core::mem::size_of::<H2>(); fn leak(_: H2) -> usize { N }", {"H1", "H2", "N"}, ["leak"]),
        ("use crate::{dtcm::{PsWakeGuardWords as Hidden}}; fn leak(_: Hidden) {}", {"Hidden"}, ["leak"]),
        ("fn leak() -> *mut u32 { (DTCM_STATE_BASE + 0x10f0) as *mut u32 }", set(), ["leak"]),
        ("const ENTRY: usize = DTCM_STATE_BASE + 0x10f0; const NEXT: usize = ENTRY; fn leak() -> *mut u32 { NEXT as *mut u32 }", {"ENTRY", "NEXT"}, ["leak"]),
        ("fn leak() -> *mut u32 { DtcmAddress::from_offset(0x10f0).cast_mut() }", set(), ["leak"]),
        ("const ENTRY: DtcmAddress = DtcmAddress::from_offset(0x10f0); const NEXT: DtcmAddress = ENTRY; fn leak() -> usize { NEXT.get() }", {"ENTRY", "NEXT"}, ["leak"]),
        ("fn leak() -> usize { DtcmAddress::from_offset_unchecked(0x10f0).get() }", set(), ["leak"]),
        ("use crate::{dtcm::{DtcmAddress as HiddenAddress}}; const ENTRY: HiddenAddress = HiddenAddress::from_offset(0x10f0); const NEXT: HiddenAddress = ENTRY; fn leak() -> usize { NEXT.get() }", {"ENTRY", "NEXT"}, ["leak"]),
        ("fn reset() { unsafe { core::ptr::write_volatile(0x0400_10e4 as *mut u32, 0) } }", set(), ["reset"]),
    )
...
def check_source() -> None:
    ...
    compile_scope = mask_test_module(source)
    focused_test = named_function(source, "ps_wake_guard_words_are_exact")
    failures.extend(
    ...
    declaration = re.search(r"#\[repr\(C, align\(4\)\)\] struct PsWakeGuardWords \{[^}]*\}", source)
    if declaration is None or normalized(declaration.group()) != normalized(STRUCT) or "derive" in declaration.group():
        failures.append("src/dtcm.rs: PsWakeGuardWords must be the exact private non-derived inventory")
    if (source.count("struct PsWakeGuardWords") != 1
    ...
    for relative, code in rust.items():
        ...
        # vendor timer_start; it reads exactly the guard word 0x0400_10e4 that
        ...
        for function in excess:
            failures.append(f"{relative}: unsanctioned production function over ps wake guard storage: {function}")
    ...
    print(f"PS WAKE GUARD WORDS SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if literals != ALLOWED_LINKED_LITERALS or xrefs != ALLOWED_DECODED_XREFS:
        raise SystemExit(f"PS WAKE GUARD WORDS LINKED DRIFT GATE FAILED\nliterals={dict(literals)!r}\nxrefs={dict(xrefs)!r}")
    residual_literals, residual_xrefs = literals - ALLOWED_LINKED_LITERALS, xrefs - ALLOWED_DECODED_XREFS
    print(f"PS WAKE GUARD WORDS LINKED DRIFT-EVIDENCE GATE PASSED allowed_literals={sum(ALLOWED_LINKED_LITERALS.values())} residual_literals={sum(residual_literals.values())} residual_xrefs={sum(residual_xrefs.values())}")
...
if __name__ == "__main__":
    try: main()
        ...
        raise SystemExit(f"initialized-ps-wake-guard drift gate failed: {error}")
