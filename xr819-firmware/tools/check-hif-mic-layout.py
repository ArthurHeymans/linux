#!/usr/bin/env python3
"""Drift-evidence gates for retained HIF and MIC queue state.
...
HIF_MIC_RANGE = (0x04009720, 0x0400993C)
...
# `0x0400980b`, and `0x04009859` words. Tracking retained owners by exact slot
# reduces those adjacent instruction words to one accidental `0x0400980c`; there
...
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter({0x0400980C: 1, 0x0400980F: 1, 0x04009814: 1, 0x0400985B: 1})
...
def in_family(value: int) -> bool:
    return HIF_MIC_RANGE[0] <= value < HIF_MIC_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: HIF/MIC literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized HIF/MIC form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"HIF/MIC SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("HIF/MIC LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "HIF/MIC LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"HIF/MIC drift gate failed: {error}")
