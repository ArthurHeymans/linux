#!/usr/bin/env python3
"""Drift-evidence gates for PHY measurement and calibration state.
...
PHY_MEASUREMENT_RANGE = (0x04009974, 0x040099AC)
...
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {
    "src/bin/hif_extension_probe.rs": {0x04009994, 0x04009998},
}
FORBIDDEN_FORMS = (
    "PHY_MEASUREMENT_BASE",
    "PHY_MEASUREMENT_STRIDE",
)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter(
    {
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware14vendor_host_tx22release_pending_to_pas', 0x040099A9): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware2tx21start_phy_operation_1', 0x040099A9): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x0400997C): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware2tx37service_single_probe_runtime_inactive', 0x040099A9): 3,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy22prepare_rf_mode0_stage', 0x0400998B): 3,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy22prepare_rf_mode0_stage', 0x040099AB): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy24begin_channel_transition', 0x04009990): 4,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy25finish_channel_transition', 0x040099A9): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy25program_all_tx_gain_slots', 0x04009994): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy29initialize_mac_software_state', 0x0400997C): 2,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy29initialize_mac_software_state', 0x040099A9): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3phy33prepare_channel_calibration_cache', 0x04009998): 1,
        ('_RNvNtCsiHlLB2CErfM_14xr819_firmware3vif12activate_sta', 0x040099A9): 1,
...
def in_family(value: int) -> bool:
    return PHY_MEASUREMENT_RANGE[0] <= value < PHY_MEASUREMENT_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: PHY-measurement literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized PHY-measurement form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"PHY MEASUREMENT SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("PHY MEASUREMENT LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "PHY MEASUREMENT LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"PHY-measurement drift gate failed: {error}")
