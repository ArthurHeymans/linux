#!/usr/bin/env python3
"""Drift-evidence gates for the PHY IQ-calibration workspace.
...
PHY_IQ_CALIBRATION_RANGE = (0x04009A0C, 0x04009C44)
...
OWNER_FILES = {
    ...
    "tools/check-phy-iq-calibration-layout.py",
}
ALLOWED_SOURCE_LITERALS: dict[str, set[int]] = {}
FORBIDDEN_FORMS = (
    "PHY_IQ_CALIBRATION_BASE",
    "PHY_IQ_CALIBRATION_STRIDE",
)
...
def in_family(value: int) -> bool:
    return PHY_IQ_CALIBRATION_RANGE[0] <= value < PHY_IQ_CALIBRATION_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: PHY-IQ-calibration literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized PHY-IQ-calibration form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"PHY IQ CALIBRATION SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("PHY IQ CALIBRATION LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "PHY IQ CALIBRATION LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"PHY-IQ-calibration drift gate failed: {error}")
