#!/usr/bin/env python3
"""Drift-evidence gates for initialized beacon/TSF/timer control words.
...
INITIALIZED_CONTROL_RANGE = (0x04001420, 0x04001440)
...
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RNvMs_NtCs2QKOjPfuf9x_14xr819_firmware14host_tx_driverNtB4_12HostTxDriver13service_index', 0x04001420): 1,
    ('_RNvMs_NtCs2QKOjPfuf9x_14xr819_firmware14host_tx_driverNtB4_12HostTxDriver5admit', 0x04001420): 1,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware14vendor_host_tx34reserve_non_aggregate_scheduler_at', 0x04001420): 2,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware2tx21start_scheduler_timer', 0x04001420): 1,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware4scan14complete_probe', 0x04001420): 1,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware4scan23publish_scan_completion', 0x04001420): 1,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware4scan5begin', 0x04001420): 1,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware4scan7service', 0x04001420): 8,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware6crypto25decrypt_rx_frame_hardware', 0x04001420): 18,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware6crypto25encrypt_tx_frame_hardware', 0x04001420): 18,
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware6crypto26run_hardware_ccmp_selftest', 0x04001420): 8,
...
def in_family(value: int) -> bool:
    return INITIALIZED_CONTROL_RANGE[0] <= value < INITIALIZED_CONTROL_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: initialized-control literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized initialized-control form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INITIALIZED CONTROL WORDS SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INITIALIZED CONTROL WORDS LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INITIALIZED CONTROL WORDS LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"initialized-control drift gate failed: {error}")
