...
checked_calls = hif.count("packet_ram::hif_dma_bus_address(")
known_references = hif.count("known_dma_bus_address(")
unchecked_calls = hif.count("packet_ram::hif_dma_bus_address_unchecked(")
if checked_calls != 2:
...
checked_definition = "pub fn hif_dma_bus_address("
unchecked_definition = "pub const unsafe fn hif_dma_bus_address_unchecked("
if packet_ram.count(checked_definition) != 1:
    errors.append("packet_ram::hif_dma_bus_address must have exactly one definition")
if packet_ram.count(unchecked_definition) != 1:
    errors.append("packet_ram::hif_dma_bus_address_unchecked must have exactly one definition")
else:
