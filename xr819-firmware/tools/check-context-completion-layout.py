#!/usr/bin/env python3
"""Drift-evidence gates for context-completion accounting state.
...
CONTEXT_COMPLETION_RANGE = (0x04008F6C, 0x04008F80)
...
FORBIDDEN_FORMS = (
    "CONTEXT_COMPLETION_BASE",
    "CLASS0_INTERNAL_CONTEXT_COUNT",
)
...
def in_family(value: int) -> bool:
    return CONTEXT_COMPLETION_RANGE[0] <= value < CONTEXT_COMPLETION_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: context-completion literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized context-completion form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"CONTEXT COMPLETION SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("CONTEXT COMPLETION LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "CONTEXT COMPLETION LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"context-completion drift gate failed: {error}")
