#!/usr/bin/env python3
"""Drift-evidence gates for initialized retained rate policies.
...
INITIALIZED_RATE_POLICY_RANGE = (0x04000200, 0x04000228)
...
FORBIDDEN_FORMS = (
    "INITIALIZED_RATE_POLICIES_BASE",
    "INITIALIZED_RATE_POLICY_STRIDE",
)

# Regenerated only after reviewing the candidate disassembly and operation order.
ALLOWED_LINKED_LITERALS: collections.Counter[int] = collections.Counter({0x04000200: 1})
ALLOWED_DECODED_XREFS: collections.Counter[tuple[str, int]] = collections.Counter({
    ('_RNvNtCs2QKOjPfuf9x_14xr819_firmware3mac31initialize_vendor_startup_state', 0x04000200): 2,
})
...
def in_family(value: int) -> bool:
    return INITIALIZED_RATE_POLICY_RANGE[0] <= value < INITIALIZED_RATE_POLICY_RANGE[1]
    ...
    for path in paths:
        ...
        for match in LITERAL.finditer(code):
            ...
            if (in_family(value) or value in SYNTHESIZED) and value not in ALLOWED_SOURCE_LITERALS.get(relative, set()):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: initialized-rate-policy literal {match.group()} is outside dtcm.rs")
        for form in FORBIDDEN_FORMS:
            for match in re.finditer(rf"\b{re.escape(form)}\b", code):
                line = code.count("\n", 0, match.start()) + 1
                failures.append(f"{relative}:{line}: synthesized initialized-rate-policy form {form} is outside dtcm.rs")
    if failures:
        raise SystemExit("\n".join(failures))
    print(f"INITIALIZED RATE POLICY SOURCE DRIFT-EVIDENCE GATE PASSED files={len(paths)}")
...
def check_elf(path: Path, dump: bool) -> None:
    ...
    if failures:
        raise SystemExit("INITIALIZED RATE POLICY LINKED DRIFT GATE FAILED\n" + "\n".join(failures))
    print(
        "INITIALIZED RATE POLICY LINKED DRIFT-EVIDENCE GATE PASSED "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"initialized-rate-policy drift gate failed: {error}")
